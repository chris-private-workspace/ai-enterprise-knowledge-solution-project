...
**Last Updated**:2026-05-16(design-mockups landed — `references/design-mockups/` high-fidelity HTML prototype + 17 `ekp-page-*.jsx` + DESIGN_README + PAGE_INVENTORY 入 repo;§5 「按需要再讀」list 加 design-mockups entry 標 frontend-first-stop + Warm Charcoal + Coral Accent oklch tokens lock per ADR-0015 W12 D2 + 3 處 design-stage expansion 待 implementation H1 ADR(KB Detail 5→8 tabs / Settings v1→6 tabs / `/users` NET NEW Tier 1.5);§5 CLAUDE.md reference v1.3→v1.5;Update history 加 row;CLAUDE.md v1.4→v1.5 同步 — §2 frontend routing first-stop entry + §7 disambiguation note `references/dify/`(third-party H3-bound)vs `references/design-mockups/`(EKP 自有 prototype,唔受 H3 但仍受 H2);§14 / Maintainer line unchanged)
