...
| **W19** | **Frontend audit + ADR drafts**(audit-and-adr-draft phase — design-mockups landed `08b74af` 2026-05-16 → user directive「先把 design-mockups 的內容 先進行實作」)— F0 kickoff cascade + F1 full mockup `.jsx` audit + F2 backend gap map + F3 ADR drafts × 5(0025–0029)+ F4 Wave breakdown(W20–W23+)+ F5 Tier 2 disabled-affordance catalog + F6 Chris approve + W20 kickoff trigger;**NO `frontend/` code change**(audit + ADR drafts + planning only) | 🟡 **active 2026-05-16**(kickoff this session;岔口 1 /users RBAC scope + 岔口 2 Settings Connections scope deferred to F6 Chris review;user 岔口 2 = mock-auth default + W22 Wave C ships mock+real both per AskUserQuestion 2026-05-16)|
| W20+ | _not pre-created_ — rolling JIT per CLAUDE.md §10 R1 | (kickoff post-W19-F6 closeout decision per ADR-0025 + F4.1 Wave A scope authorization;candidates = W20-frontend-wave-a(`/dashboard` + `/chat` + `/kb` + `/kb/[id]` 8-tab base)/ W21-frontend-wave-b(`/doc-detail` + `/eval` + `/traces/[traceId]`)/ W22-frontend-wave-c(`/settings` + `/users` + auth polish + real-MSAL ship + backend extensions per ADR-0026/0027)/ Wave D Tier 2 hold post-Beta Q12;W16 F1-F4 Track A IT cred remains parallel track,mock-auth default continues through Wave C per user 岔口 2)|
...
**Last Updated**:2026-05-16(W19-frontend-audit-and-adr-draft phase kickoff — design-mockups → real-`frontend/` implementation Phase work triggered by user directive same-day post-`08b74af`;§10 W19 row added(`active`,F0-F6 audit + ADR drafts + planning,NO `frontend/` code change)+ W18+ → W20+ shift(W20-frontend-wave-a Wave A candidate / W21-frontend-wave-b / W22-frontend-wave-c — mock-auth default + real-MSAL both ship per user 岔口 2 decision / Wave D Tier 2 hold);ADR-0025–0029 reserved in `docs/adr/README.md`(KB Detail 8-tab / Settings 6-tab + Connections / `/users` Tier 1.5 RBAC / `/kb/new` 5-step / `/doc-detail` 3-pane — Status `Reserved (W19 F3)`,content drafted in W19 F3,flip to Proposed → Accepted at W19 F6 Chris review);Next NNNN 0025 → 0030;§11 / §12 unchanged at kickoff(retro + milestones row land at W19 F6 closeout per W18 pattern);Update history 加 row;CLAUDE.md unchanged this update — W19 是 plan-phase 而非 H1/H2/H3 改動)
