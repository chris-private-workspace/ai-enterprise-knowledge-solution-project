...
- ⏸ **R8** Ricoh corp proxy — **5 cumulative occurrences**(Cohere W3 Marketplace + argon2-cffi W13 ADR-0016 stdlib switch + ACS SDK W13 lazy import + Playwright browser CDN W15 D5 ECONNRESET + psycopg W17 F1 IncompleteRead);**ADR-0017 landed 2026-05-10 W17 F0b** formalizing the mitigation pattern. **Plan B realised 2026-05-13**(ADR-0017 §"Plan B realised"):the Playwright-browser-CDN block is now sidestepped on the corp box itself via `PW_CHANNEL=chrome`(system Chrome)→ E2E 15/15 green + 4 pixel baselines captured(CO_W15_F4_browser_binaries + _baseline_capture CLOSED). **Converse confirmed W17 F6**:`pnpm add -D` of npm-registry deps goes through fine — R8 blocks **binary-CDN** downloads(`cdn.playwright.dev`, large PyPI binary wheels), not the npm/PyPI metadata/registry. **Residual 🚧**:`pip install psycopg[binary]`(F1.5b — re-attempted 2026-05-13, PyPI wheel timed out)— the only binary-CDN item still without a workaround;`pip` mirror = IT-side fix(out of scope)
...
**Last Updated**:2026-05-11(W18-app-shell-ia closeout housekeeping catch-up — §3 C09 status W12-W15 9 views → **W18 unified application shell IA per ADR-0024**(`<AppShell>` across all authenticated views + URL flatten `/admin/*`→`/kb/*` / `/debug/[traceId]`→`/traces/[traceId]` + `/dashboard` real overview + `/settings` + `<GlobalSearch>` Cmd/Ctrl+K palette + V7 Landing removed `/`→redirect + login/register→`/dashboard` + Vitest 4/13 + `[oklch`=0 preserved)+ C10 status(chat view in `<AppShell>` now, `<main>`→`<div>`, reads `?q=` on mount, route `/chat` unchanged);§10 W18+ row replaced by **W18 closed 2026-05-11**(Gate PASS WITH SMOKE-USER-DEFERRED CAVEAT)+ W19+ not-pre-created;§11 "已知未解" header W17 closeout → W18 closeout + W18 CLOSED block(ADR-0024 implementation — D1-D10 all landed)+ Narrowed-by-W18(CO_W15_F3_dark_mode + CO_W15_F4_interactive_flow_E2E — interactive 9-view-in-shell walkthrough + full Playwright run = user pre-Beta smoke, R8 browser-binary block)+ NEW W18-closeout follow-ups(`<LoginGate>` `// TODO(W16)` + dashboard "system health"→richer `/health` + dashboard "recent queries"/"latest eval" CTAs→backend + local-dev seed-KB W19+ candidate);§12 milestones W18-app-shell-ia row added + 累計 16→17 phase closed;ADR-0024 D1-D10 ticked + Status verified Accepted;§9 OQ unchanged(no new OQ W18 — the multi-language affordance is a known §11 Tier 2 disabled affordance);next ADR NNNN still 0025)
> **Prior**:2026-05-10(W17-beta-hardening closeout housekeeping catch-up — §2 "v5 frozen"→v6 + Cohere v3.5→v4.0-pro + ACS/Postgres added to the locked-vendor list;§3 C02 in-memory→Postgres-backed(ADR-0023, CO18 CLOSED)+ C06 RAGAs integrated(W17 F3)+ C08 stub cascade DONE + Implemented + C11 cookie/CSRF transport(ADR-0022, CO_F5_* CLOSED)+ users_repo Postgres-backed;§4 authority-order header v5→v6;§10 W16 row note(F5 done, F1-F4 Track-A-blocked)+ W17 row added(closed, Gate PASS)+ W18+ not-pre-created;§11 carry-overs replaced W15 D5 → W17 closeout(W17 CLOSED block: CO18 + CO_F5_* + CO_W15_F1_backend + CO_W15_F2_* + CO_F3a-c + CO_W15_F4_vitest;partially-closed CO_W15_F3_dark_mode;NEW 🚧 F1.5b + F3.5b under CO17;OPEN CO_W15_F1_eval_set_v1)+ ADR status 15→22 landed + next-NNNN 0017→0024(ADR-0017 R8-pattern + 0018-0021 audit batch + 0022/0023 W17 F0)+ R8 4→5 occurrences(ADR-0017 landed)+ converse-confirmed(npm registry not blocked);§12 milestones W16 row updated + W17 row added;累計 15→16 phase closed;§9 OQ unchanged(no new OQ W17))
