...
| **C09** | Admin Console UI(Next.js + shadcn/ui)| W1 D1 | ✅ Implemented(W2 partial → **W12-W15 UI Tier 1 expansion 9 views per ADR-0015** → **W18 unified application shell IA per ADR-0024** → **W20 Wave A 8-route refactor per ADR-0025/0028/0031**:F1 `<NotificationsMenu>` + Workspace switcher disabled affordance + Sidebar Tools sub-section + Labs hidden + NEW shared `<DisabledAffordance>`(W19 F5 spec)/ F2 `/dashboard` 4-stat strip + 5 cards + per-component dots / F3 `/chat` advanced surfaces 7 NEW components(ConversationHistory + InlineImageCard + ImageGallery + CitationPill + FeedbackBar + CragStrip + rewrite)+ 3 citation modes / F4 `/kb` grid+table+filter + `/kb/new` 5-step wizard / F5 `/kb/[id]` 7-tab `-Access` refactor(Documents+Chunks+Images NEW+Chunking Lab NEW+Pipeline+Retrieval Testing+Settings;Access tab disabled affordance OUTSIDE VALID_TABS — Wave C1 activates)/ F6 `/kb/[id]/upload` single-step → 3-step rebuild / F7 `/login` strict-fidelity refactor + `/register` polish per ADR-0014 + CLAUDE.md §3.2.1 H7;Vitest 6→14 files/21→37 tests + Playwright +7 NEW E2E test cases + 2 NEW visual baselines;`[oklch(...)]`=0 milestone preserved through F1-F8 + 9 commits)|
| **C10** | Chat Interface UI(Next.js + Vercel AI SDK)| W3 D2 | ✅ Implemented(W3 streaming + Citation + W13 routing restructure + theme provider + dark mode toggle + W15 token cleanup + **W18(ADR-0024)** chat view renders inside `<AppShell>` + `?q=` deep-link + **W20 F3 advanced surfaces per ADR-0031 Option B**(promotes C10 §7 Tier 2 → Tier 1):**server-side Conversation History sidebar**(Postgres `conversations` + `messages` per ADR-0023 backing + 6 `/conversations` CRUD + InMemory fallback;12/12 pytest)+ 3 citation placement modes(`inline` / `footnote` / `sidebar` toggle persisted to `localStorage['ekp-citation-mode']`)+ NEW `<InlineImageCard>` + NEW `<ImageGallery>`(aggregates `citations.embedded_images[0]` across conversation)+ NEW `<CitationPill>`(vanilla popover w/ hover-grace + keyboard a11y)+ NEW `<FeedbackBar>`(thumbs + tag dropdown + textarea;tag prefix `[tag] text…` into existing W8 `POST /feedback` `comment` field — no schema change)+ NEW `<CragStrip>`(dormant Tier 1 wire);SSE `streamQuery` preserved verbatim — persistence layer additive only;route `/chat` unchanged)|
...
| W21+ | _not pre-created_ — rolling JIT per CLAUDE.md §10 R1 | (kickoff post-W20 closeout decision **2026-05-17**;**candidates**:**W21-frontend-wave-b**(`/doc-detail` 3-pane `/kb/[id]/docs/[docId]` per ADR-0029 Option C + `/eval` Eval Console refactor + `/traces` index + `/traces/[traceId]` 3 viz modes per ADR-0030 absorbed;backend ~2 days + frontend ~6.5-7.5 days;window ~1 week)/ **W22-frontend-wave-c1 + W22b-frontend-wave-c2 SPLIT per F4 §3.6 trigger**(Wave C scope decision at W22 kickoff;ADR-0026 Option B + ADR-0027 Option A combined ~42 backend days exceeds single phase budget;activates `/kb/[id]` Access tab per ADR-0025;ships `/settings` 6-tab + `/users` Tier 1.5 RBAC + real-MSAL feature-flagged concurrent ship per user 岔口 2;**2 NEW dependencies** Key Vault SDK + Entra Graph SDK Plan B sequencing decision per ADR-0017)/ **Rule-of-3 wizard primitive promotion**(shared `<Stepper>` + Field/Stage extract per W20 F6.2 trigger — 4th wizard usage hit:F4 KB Pipeline + W13 Register + W18 F5 Pipeline + W20 F6 Re-ingestion) / Wave D Tier 2 hold post-Beta Q12 / W16 F1-F4 Track A IT cred remains parallel track,mock-auth default continues through Wave C per user 岔口 2)|
...
### 已知未解(at session start time = 2026-05-17 W20-frontend-wave-a closeout)
...
| W21+ | _not pre-created_ | _pending W20 closeout decision 2026-05-17_ | (rolling JIT — W21+ phase folder NOT pre-created per CLAUDE.md §10 R1;candidates per W19 F4 Wave breakdown = **W21-frontend-wave-b**(Doc Detail 3-pane per ADR-0029 Option C `/kb/[id]/docs/[docId]` + Eval Console + Traces index + Traces 3 viz)/ **W22-frontend-wave-c1 + W22b-frontend-wave-c2 SPLIT per F4 §3.6 trigger**(Option A+B Wave C scope ~42 days exceeds single phase;Settings 6-tab + /users Tier 1.5 RBAC + Access tab activate + real-MSAL feature-flagged concurrent ship;**2 NEW deps** Key Vault SDK + Entra Graph SDK Plan B sequencing decision per ADR-0017)/ **Rule-of-3 wizard primitive promotion**(shared `<Stepper>` extract per W20 F6.2 trigger)/ Wave D Tier 2 hold post-Beta Q12;W16 F1-F4 Track A IT cred parallel track,mock-auth default continues through Wave C per user 岔口 2)|

**累計**:**19 phase closed**(W1-W15 + W17 + W18 + W19 + **W20 closed 2026-05-17** — Phase Gate PASS WITH SMOKE-USER-DEFERRED CAVEAT;F0-F8 all landed across 10 commits + F9 closeout;Tier 1 12-week sprint extended via W12-W15 UI sprint cycle pivot — architecture v5.1 → v6 amendment + 9 views — then W17 Beta-hardening parallel to W16, then W18 unified application shell IA per ADR-0024, then **W19-frontend-audit-and-adr-draft** plan phase for design-mockups → real-`frontend/` implementation with 6 ADRs Accepted(0025/0026/0027/0028/0029/0031), then **W20-frontend-wave-a** shipped Wave A `/dashboard` + `/chat` advanced + `/kb` cluster 7-tab + auth/topbar polish via ADR-0025/0028/0031 + ADR-0030/0032 absorbed scope);W21+ rolling JIT pending closeout decision;**W16 status:draft**(F5 backend stub closure cascade DONE;F1-F4 Track-A-blocked pending IT cred populate event + R-B1 closure — parallel-track to W17/W18/W19/W20)
