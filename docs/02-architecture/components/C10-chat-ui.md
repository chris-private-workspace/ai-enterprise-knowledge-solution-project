...
> **CH-009 amendment(2026-06-08,Chat image relevance per ADR-0046)**:chat 圖片 display 層三項(`frontend/lib/chat/citation-images.ts` + `chat/page.tsx`)—— ① **Decorative filter(OD-1)**:`dedupeCitationImages` skip `isDecorativeImage`(`min(width,height)<DECORATIVE_MIN_PX=64`,dims 已知先判;0×0 legacy 保留),走手冊 Tip/Note 燈泡 icon(dims 由 C01 ADR-0046 probe 落 index)。② **Per-KB cap(OD-2)**:`MessageRow` cap = `maxImagesPerAnswer ?? INLINE_IMAGE_CAP`(`ChatThread`→`MessageRow` thread;chat page 讀 `activeKb.config.max_images_per_answer ?? null`)。③ **Relevance-select(OD-3)**:`selectInlineImages(deduped, cap)` 揀 owning citation `relevance_score`(max across citations,Cohere 文字 rerank 信號)最高 cap 張 → cap 內 document-order(Finding D)顯示;`dedupedImages.length`(全 deduped)仍驅動「View all in Image Library」。**H4:純文字信號,無 image embedding**。Live `/query` 驗:40 unique → 1 個 93×62 燈泡判 decorative、39 內容圖保留、dims 0 miss。
