...
**2026-05-12 (CH-001) — Phase 3 upload-side CLOSED**:`POST /kb` auto-provisions `ekp-kb-{kb_id}-v1` via `IndexPopulator.create_index_for_kb` (PUT from `backend/indexing/schema.json` canonical schema) + storage-rollback on Azure failure;`DELETE /kb` drops the index via `IndexPopulator.delete_index` (fail-soft on 404);`IndexPopulator.upload(records, kb_id=None)` BC sig ext routes via `kb_id_to_index_name` when kb_id provided + falls back to `self.index_name` for legacy callers (`scripts/run_populate_sanity.py` unchanged);the document routes (POST upload / DELETE doc / POST reindex per Decision A=(ii) replace-in-place) target per-KB indices end-to-end. **Phase 3 blob-container-side stays deferred** — per-KB `ekp-kb-{kb_id}-screenshots` provisioning blocked on R12 Azurite signature mismatch + W16+ Track A cloud Blob switch;ADR-0018 Status remains `Accepted` (NOT `done`) until blob-side closes too. CH-001 spec:`docs/03-implementation/changes/CH-001-wire-document-upload-delete-reindex/spec.md` §6.6 + R11.
