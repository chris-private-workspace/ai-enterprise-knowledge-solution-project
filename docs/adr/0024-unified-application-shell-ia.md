...
> Implemented by the `W18-app-shell-ia` phase (`plan.md` per CLAUDE.md §10 R1; folder created on acceptance 2026-05-10). **All D1–D10 landed — W18 closed 2026-05-11, phase Gate = PASS WITH SMOKE-USER-DEFERRED CAVEAT** (the multi-viewport / dark-mode browser walkthrough + the full Playwright run stay the user's pre-Beta smoke — R8 `npx playwright install chromium` blocked / CO_W15_F4_browser_binaries / ADR-0017; the spec updates + `tsc` compile-check are the deliverable). The phase `plan.md` mapped these D1–D10 onto its F-numbered scope (F1=D1 / F2=D2+D7 / F3=D3 / F4=D4 / F5=D5 / F6=D6 / F7=D8 / F8=D9 / F0+F8.6+F9.6=D10).

- [x] D1 — `frontend/components/nav/app-shell.tsx` (generalized `admin-shell.tsx`): collapsible left sidebar (5 modules: Dashboard / Chat / Knowledge Bases / Eval Console / Traces, `aria-current="page"`) + top bar (app name "EKP" → `/dashboard`, global-search trigger + Cmd/Ctrl+K, disabled language toggle, `<ThemeToggle>`, `<UserMenu>`) + main content slot + responsive hamburger drawer (`<Sheet>`) + focus-mode toggle (persisted to `localStorage`); 100% token classes, `[oklch`=0 — **W18 F1**
- [x] D2 — `app/(app)/layout.tsx` route group: `<AuthProvider><QueryProvider><LoginGate><AppShell>{children}</AppShell></LoginGate></QueryProvider></AuthProvider>`; removed `app/admin/layout.tsx` / `app/eval/layout.tsx` / `app/debug/layout.tsx` / `app/admin/page.tsx` (+ orphaned `admin-shell.tsx`) — landed atomically with the F3 page move (physically inseparable) — **W18 F2 (layout removals → F3)**
- [x] D3 — moved + re-routed pages into `(app)/`: `chat`; `kb` (from `admin/kb`, `/admin/` dropped); `kb/[id]`; `kb/new`; `kb/[id]/upload`; `eval`; `traces/[traceId]` (from `debug/[traceId]`); `app/admin/error.tsx` → `app/(app)/error.tsx`; + 2 thin new pages (`(app)/dashboard` placeholder → F4 fills it; `(app)/traces` index); updated **all** internal `<Link href>` + `router.push` + the Playwright `tests/e2e/` route refs + `.gitignore` `traces/`→`/traces/` (the bare pattern would have shadowed the new route folder); `next.config.mjs` verified path-agnostic — **W18 F3 (IA flip live)**
- [x] D4 — `app/(app)/dashboard/page.tsx` — real overview cards: KB summary (off `GET /kb`), recent-queries CTA (no log endpoint — Q6), latest-eval CTA (no cached-run endpoint), system-health (off `GET /health` = backend liveness), quick actions (New KB / Upload / Run Eval / Open Chat); no new backend — **W18 F4**
- [x] D5 — `app/(app)/settings/page.tsx` — profile display (`preferredUsername` / `oid` / `tid` + mock badge — `AuthenticatedUser` has no separate display-name/email field) + theme preference (`<ThemeToggle>`) + sign-out (`useAuthStore.signOut`); `<UserMenu>` got a "Settings" item — **W18 F5**
- [x] D6 — `frontend/components/nav/global-search.tsx` — shadcn-`Dialog`-based Cmd/Ctrl+K command palette (**zero new dep** — `cmdk` not needed): Pages (the 5 modules + Settings, static) + Knowledge bases (off the cached `GET /kb`) + always-present "Ask in chat: …" → `/chat?q=…`; arrow-key nav + `aria-activedescendant`; recent-docs/recent-traces dropped (no cheap backend source — the Pages results are the Tier-1 quick-jump set); `chat/page.tsx` reads `?q=` on mount (pre-fill) — **W18 F6**
- [x] D7 — login-gate: `<LoginGate>` in the `(app)/` layout — mock-auth dev = no-op pass-through; real MSAL = a "Sign in to continue" splash + `<Link href="/login">` (a gate-screen, not an auto-`redirect()` — matches the existing `AuthProvider` anti-infinite-loop design; a `// TODO(W16)` to tighten once Q11 cred wiring is live) — **W18 F2**
- [x] D8 — `app/login/page.tsx` (`handleSelfSubmit` + `handleSsoClick`) + `app/register/page.tsx` (Step 3 CTA) → `router.push('/dashboard')` (was `/chat`); `app/page.tsx` — the V7 Landing markup **deleted** → thin server `redirect('/login')` (always → `/login`; the `(app)/` login-gate + MSAL re-route authenticated users); `brand-panel.tsx` kept (nothing orphaned) — **W18 F7**
- [x] D9 — responsive + a11y pass: `<AppShell>` `aria-current` / `aria-expanded` / `aria-pressed` + `<nav aria-label="Primary">` (verified in place); `<GlobalSearch>` `role="dialog"`+`aria-modal`+focus-trap+Escape (Radix) + `role="combobox"`+`aria-activedescendant` + `role="listbox"`/`option` (verified); `role="heading" aria-level={2}` added to the dashboard (6) + settings (3) `CardTitle`s; `[oklch`=0 re-checked (milestone preserved through F1-F8); 3 NEW Vitest files (`app-shell` / `global-search` / `dashboard`) → `pnpm test:unit` 4 files / 13 tests pass (was 1/3); Playwright `app-shell-path.spec.ts` updated (real `/dashboard` test + a "shell present on app routes / absent on auth pages" assertion) + `tsc` compile-check; the multi-viewport / dark-mode browser walkthrough + the full Playwright run stay the user's pre-Beta smoke (R8 / CO_W15_F4_browser_binaries) — **W18 F8**
- [x] D10 — `architecture.md v6 §5` amendment landed at W18 kickoff (inline-tagged, doc version held — §5.0 Application Shell added / §5.3 "Admin Dashboard"→"Dashboard" / §5.7 "Debug View"→"Traces" / §5.9 V7 Landing → REMOVED tombstone / §5.2 Chat in-shell / `/admin/*`→`/kb/*` flatten / §5.10-§5.11 Login/Register redirect target); ADR-0015 References got the "amended by ADR-0024" note; `COMPONENT_CATALOG.md` C09/C10 `Status` rows + `session-start.md §3` C09/C10 + §10 W18 row + §11 carry-overs + §12 milestones updated at W18 closeout; `decision-form.md` — no new OQ surfaced (the multi-language affordance is a known §11 Tier 2 item) — **W18 F0 (kickoff) + F8.6 + F9.6**
