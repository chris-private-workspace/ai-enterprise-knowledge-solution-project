...
**Date**: 2026-06-03
**Status**: **Proposed**(pending Chris confirm H1 boundary + 決策 2 切法 + Accept — W44 F0 gate)
...
**(本節為 proposed recommendation,最終切法待 W44 F0 gate Chris 拍板 — 決策 2)**

令圖片**隨內容切分按 `doc_order` 分配落 sub-chunk**,而非 pile-on 整個 section:

1. **Image-aware flush(recommended 切法 A)**:`_SectionAccumulator` 加 image-count 感知;當累積圖數達 `max_images_per_chunk` soft cap,**force flush** 當前 chunk(連已累積 text + 嗰批圖),開新 sub-chunk(延續同一 `section_path`,prev/next 連續)。圖按 `doc_order` 跟最近 flush 分配 → text flush 同步 reset `image_positions`(修 `:207`/`:213-218` 嘅 pile-on)。
...
3. **新 `Settings.chunker_max_images_per_chunk` knob**(default 待 gate 定;`None`/0 = 不限 = 今日 bit-identical 行為,measurement-first preserve)。後續可循 ADR-0040 模式降至 per-KB `KbConfig`(W44 暫不做,守 simplicity)。
