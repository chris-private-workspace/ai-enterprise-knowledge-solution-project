...
> **Amendment(Settings page scope expansion)**:per ADR-0026 Option B fully editable(W19 F6 Chris pick 2026-05-16) — `/settings` 由 ADR-0024 v1 thin(profile display + theme toggle + sign-out)**promoted 到 6-tab hub**:Profile / Appearance / **Connections**(5 categories × 9 providers with ServiceCard + ApiKeyInput + DeploymentsTable;Test connection + Rotate secret per provider via Key Vault SDK)/ **Identity & Auth**(Entra tenant + App registration + MSAL config + Role mapping[3 active + Power User Tier 2 disabled affordance per ADR-0027 fallback]+ Sign-in policy)/ **API Keys & Quotas**(4-stat usage strip + per-provider TPM/RPM quotas;Incoming API keys Tier 2 disabled affordance「Tier 1 access via web UI only」)/ Account。**Beta-readiness milestone**:`.env` rotation rituals + Azure Portal trips 取代為 self-service UI。**Triggers**:(a) Postgres NEW tables `admin_provider_configs` + `admin_identity_config` + `audit_log`(per ADR-0023 base — additive,idempotent ALTER TABLE);(b) NEW `/admin/*` route group `/admin/connections/*` + `/admin/identity/*` + `/admin/api-keys/*` + `/admin/usage-stats`(~22 endpoints — Wave C1 backend scope);(c) NEW dependency `azure-keyvault-secrets>=4.8.0 azure-identity>=1.18.0` per ADR-0017 Plan B (c) mobile hotspot mitigation(occurrence #8);(d) NEW `KeyVaultProvider` Protocol(EnvVarProvider fallback when `KEY_VAULT_URL` unset + AzureKeyVaultProvider production path per ADR-0023 backend-Protocol pattern)。Inline-tagged amendment(doc version held — 同 §3.4 / §3.7 / above ADR-0024 amendment convention);ADR-0026 係 authoritative record。Implementation = `W24-frontend-wave-c1` phase(`plan.md` per CLAUDE.md §10 R1)。Access tab activate(per ADR-0025) + `/users` 4-tab(per ADR-0027 Option A full)defer Wave C2(`kb_acl` Postgres table + Entra Graph SDK + ACL middleware + 6 NEW tables ~20 backend days)。

> **Amendment(/users Tier 1.5 RBAC + Access tab activation)**:per ADR-0027 Option A full RBAC(W19 F6 Chris pick 2026-05-16) — RBAC 由原本「Tier 2 hook」(role enforcement deferred)**promoted 到 Tier 1.5 minimum**:NET NEW `/users` 4-tab page — **Members**(列表 + filter seg + invite / suspend / role-change)/ **Roles & permissions**(3 active roles Admin·Editor·End User + Power User Tier 2 disabled affordance + PERMISSIONS_MATRIX 5 areas × 24 permissions)/ **Groups**(Entra ID group → EKP role sync)/ **Audit log**(activity feed 6 action types + 90d retention)— plus per-KB ACL `TabKbAccess`(Manage / Edit / Query role override),activating the `/kb/[id]` **Access tab**(disabled affordance per the §5.5 ADR-0025 amendment → active)。**Triggers**:(a) **5 NEW Postgres tables** `roles` + `role_permissions` + `groups` + `group_members` + `kb_acl`(per ADR-0023 base — additive,idempotent)+ `users.role` column ADD;**注**:`audit_log` table 已存在(ADR-0026 W24-c1 F4 created + W24b F6 filter/pagination)→ W24c additively EXTENDs 其 `AuditAction` Literal,唔 re-create — ADR-0027 原寫「6 NEW tables」實際 **5 NEW**(W24c F0 R6 finding);(b) NEW **C16 Users Service** component(authorization + user-management concern,distinct from C11 authentication — W24c F1.3 decision over「fold into C11」);(c) ACL middleware(`@requires_role` / `@requires_kb_acl` decorators)+ auth-time role claim(session / cookie carries role;mock-auth `role:'admin'`,real-MSAL Entra group → role session callback);(d) `POST /groups/sync-from-entra` via **managed-REST**(既有 `azure-identity` token + `httpx` `GET graph.microsoft.com/v1.0/groups`)— **no Entra Graph SDK new dependency**(ADR-0027 原寫 SDK;W24c F1 pick managed-REST per ADR-0017 §Decision-rule「managed-REST > heavy SDK」+ zero H2 / R8 — Chris AskUserQuestion 2026-05-21)。**H4 boundary**:custom role creation + Power User role activation + multi-tenancy stay Tier 2。Inline-tagged amendment(doc version held — 同 §3.4 / §3.7 / above ADR-0024+0026 amendment convention);ADR-0027 係 authoritative record(其 §Decision「amend §3.7 + add §3.8」嘅 §-pointer 經 W24c F1 R6 audit 更正 — §3.7 實為 C13 Email Verification Service,`/users` 屬 UI view 故 amendment 落 §5,對齊 ADR-0024/0025/0026 §5 inline-amendment convention)。Implementation = `W24c-users-rbac` phase(`plan.md` per CLAUDE.md §10 R1)。
...
> **W48 amendment per ADR-0040(dual-axis fulfilled)**(F1-F3):config-test 試跑 panel 加 **reference-free RAGAs faithfulness 質素軸**(反幻覺;補 ADR-0040 一早定義但未交付嘅 quality 軸 — 原本只有 presentation counters)。`ConfigTestRequest.eval_faithfulness`(default true)+ `ConfigRunSummary.faithfulness`(用 last-run answer + retrieved contexts;**W49 改 N-run band,見下**);A/B 兩卡各顯示「忠實度」headline。NEW **additive** `make_faithfulness_evaluator(settings)`(`backend/eval/ragas_evaluator.py`,faithfulness-only,judge 維 `gpt-5.4-mini`;現有 `make_ragas_evaluator` 4-metric 不動)。Graceful:無 judge / error → `None`(panel 顯示「—未評」,config-test 唔 fail)。**config-test 仍 query-pipeline-only**:ingestion config(`chunk_strategy`/圖數cap)質素需 reindex→eval,屬未來期(out-of-scope)。

> **W49 amendment per ADR-0040 + 決策 7(quality-axis noise hardening)**(F1-F3,H7 design-first):W48 faithfulness **每 config 只算一次**(last-run)→ single-shot 噪音大(2026-06-06 live:同一 SAVED config 兩跑 0.929↔0.53)。W49 令 faithfulness **逐 run judge 並聚合成 `MetricBand`**(`ConfigRunSummary.faithfulness: float|None` → **`MetricBand|None`**,同 presentation counters 一致)——`backend/api/routes/config_test.py` NEW `_faithfulness_band()`:`asyncio.gather(asyncio.to_thread(faithfulness_fn,...))` 並發逐 run judge,band over judge 成功嘅 run(全 fail / 軸關 → `None`)。**judge 成本 = 用戶揀嘅 `runs`(1-5)**,無新 toggle(per AskUserQuestion Option 1)。Panel headline:N≥2 顯示 `mean ± band`(揭噪音);**N=1 顯示單值 + 「單次 judge · 方向性」warning**(`summary.runs.length === 1`)。判 LLM 維 `gpt-5.4-mini`;無新 ADR(extend ADR-0040 雙軸)。**已知 limitation**(roadmap 證據②):RAGAs faithfulness 對長/全面答案有 length bias,可能同 completeness 反相關 → 列舉型 query 解讀需配 recall 對沖(留未來)。

> **W50 amendment per 決策 7 Option (d)(length-bias affordance — 平 part)**(F1-F2,**frontend-only**,H7 design-first):收 W49 known limitation 嘅教育半邊。config-test 試跑 panel 加 **length-bias explainer affordance**:(a)「忠實度」headline label `title=` tooltip(faithfulness 性質 + length bias 警示);(b) A/B 結果 grid 下方單一 **caveat note**(`AlertTriangle` warning 色 + 「忠實度對長/全面答案有 **length bias** …勿與完整性混為一談」)。**static explainer 無 arbitrary threshold**;用 result card 現有資料(無 backend / 無新 metric / 無新 dep / 無新 ADR)。**completeness/recall 對沖指標**(roadmap Option d「ideal」半邊)留 W51。

> **W51 amendment per 決策 7 Option (d) ideal 半邊(completeness proxy)**(F1-F3,H7 design-first):W50 加咗 length-bias 文字 caveat,W51 加**量化 completeness 訊號**畀用戶直接睇到 faithfulness↓/completeness↑ trade-off。NEW **「涵蓋章節數」(distinct-sections-cited)per-run band**:`RunMetrics.distinct_sections: int` + `ConfigRunSummary.distinct_sections: MetricBand`;`_run_n` 逐 run `len({tuple(c.section_path) for c in resp.citations})`(`Citation.section_path`)→ 沿用既有 `_band()`(零新 model/helper)。Frontend:result card grid 加「涵蓋章節數」metric(擺 引用數 旁,coverage 訊號群組喺 faithfulness 下)+ W50 caveat 更新指向佢。**誠實 framing(R1)**:label 標清係 **coverage proxy(廣度)非 ground-truth recall**(無標註集真 recall 計唔到;sub「completeness proxy · 非 recall」+ docstring 標明)。**synthetic-QA recall** 留更未來(W52 落 offline harness,見下)。無新 ADR/vendor;judge 不涉(distinct-sections 純 citation metadata 算)。

> **W52 amendment per 決策 7 Option (d) 更深半邊(synthetic-QA 真 recall — offline 工程閘,非 config-test panel)**(F1-F3,backend-only **無 H7**):收 W51 forward-pointer。W51「涵蓋章節數」係 coverage proxy,W52 補返**真 recall 半邊**,但**唔係**放 config-test 試跑 panel(ROADMAP line 107「synthetic-QA auto-gen → 留工程閘」)—— NEW offline harness `backend/eval/synthetic_qa.py`(C06):由 KB indexed chunk 用 LLM(gpt-5.4-mini)生成問題(source chunk = ground truth)→ formatted 成 `acceptable_chunk_ids=[source]`+`validated=True` 餵現有 `EvalRunner` strict-mode(**零新 recall 數學**)→ self-supervised Recall@K。Reuse judge client pattern(`patch_for_gpt5`)+ chunk 枚舉(`list_documents`/`list_chunks`/NEW `RetrievalEngine.fetch_by_chunk_ids` passthrough)。Live driver `scripts/run_synthetic_recall.py`。**誠實 framing(R1)**:**self-supervised synthetic recall,非人手 ground-truth recall**(LLM 生成問題 + 單 chunk grounding 偏差;用途 = 相對比較,非絕對 verdict)—— 詳 bias disclosure 見 `eval-methodology.md §10.6`。無新 ADR/vendor/dep。W53 候選:用此 recall 指標跨 chunk strategy reindex 比較(rolling JIT)。

> **W53 amendment per ADR-0044(兩者合一下半截 — heading_aware 真 strategy + chunk-strategy recall 比較)**(F1-F4;**chunker 改動屬 §3.3/§3.5 per ADR-0044**,非 config-test panel):用 W52 synthetic recall 跨 `chunk_strategy` 比較 ingestion 質素。R6 grep 揭三發現(① `chunk_strategy` degenerate:`auto`/`layout_aware`/`slide_based` 全 → LayoutAwareChunker,`heading_aware` 未實作;② reindex `_select_chunker` 只睇 image-cap IGNORE chunk_strategy;③ 跨 reindex chunk_id 變)→ **(F2 ADR-0044)** 實作 `HeadingAwareChunker`(`backend/ingestion/chunker/heading_aware.py`,thin `LayoutAwareChunker` subclass:`target_tokens=hard_cap`[取消 sub-target split] + `min_chunk_merge_floor=0`[取消 merge],仍保 hard_cap split + image-cap[ADR-0041]→ 更粗/少 chunk)+ wire `chunk_strategy` 入 `strategies.select_chunker` + `documents.py _select_chunker`(strategy≠heading_aware bit-identical fall-through;**順帶 close W46 reindex chunk_strategy over-promise gap**)。**(F3)** NEW `backend/eval/strategy_comparison.py` `run_strategy_recall_comparison`(per strategy:set chunk_strategy → reindex → `run_synthetic_recall` → 收集;**零新 recall 數學**)+ live CLI `scripts/run_strategy_recall_comparison.py`。**誠實 framing(R1/R2)**:per-config 重生 QA → 問題集每 strategy 唔同 → **self-retrievability(自檢索性)非 controlled A/B**,且建基 W52 synthetic recall(非人手 ground truth)。無新 vendor/dep。W54 候選:controlled shared-question A/B(text-anchored)/ per-document scope。

> **W54 amendment(W53 self-retrievability 嚴謹版 — controlled shared-question A/B)**(F1-F4,backend-only **無 H7 無 H1**;純 C06 eval extension):收 W53 明文 deferred 嘅 controlled A/B。W53 per-config 重生 QA → 每 strategy 問題集唔同 → recall 差異 confounded(self-retrievability);W54 用**一個 frozen question set 跨所有 strategy 評分** → 消除該 confounding。R6 grep 揭三發現(① `EvalRunner` 已內建 **keyword-mode recall**[`runner.py:166-184`:`validated=False`/無真 chunk_id → `|expected_answer_keywords ∩ top-K chunk_text| / |keywords|`,chunk_id 無關]→ reuse 零新 recall 數學;② `section_path` strategy-invariant[`HeadingAwareChunker` 繼承同一 section-walk]→ 做文字錨點;③ W53 dataclass self-retrievability framing 不可 reuse → W54 自有 controlled dataclass)。NEW `backend/eval/controlled_comparison.py`(C06):reference index → `_collect_chunks`(reuse W52)→ 按 `section_path` group section passages(strategy-invariant 文字錨點)→ judge LLM(gpt-5.4-mini)逐 passage 生**一問題 + 3-5 verbatim answer keywords** → `build_shared_eval_set` 寫**一個 frozen** keyword-mode eval-set YAML → `run_controlled_strategy_comparison`(per strategy:reindex → `EvalRunner` keyword mode 跑**同一 frozen set** → 收集)。Live CLI `scripts/run_controlled_ab_comparison.py`。**誠實 framing(R1 — 誠實階梯第四期)**:本期 **IS** controlled(shared frozen set 消 confounding),**但仍**(1) synthetic(LLM 問題+keywords,非人手 ground truth)+(2) **keyword-containment = lexical proxy**(含詞 chunk ≠ 正確 answer-bearing chunk;section-anchored 問題可被多 chunk 答中 → 偏寬鬆)→ **相對信號非絕對 verdict**(詳 `eval-methodology.md §10.6`)。無新 ADR/vendor/dep。W55+ 候選:人手標註 ground-truth recall / per-document scope。
