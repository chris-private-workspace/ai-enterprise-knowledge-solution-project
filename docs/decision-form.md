...
| **Decision** | **Azure OpenAI 完整 deployment 已 ready**。**W1 D2 (2026-05-01) implementation detail delivered**:endpoint = `https://chris-mj48nnoz-eastus2.cognitiveservices.azure.com/`,region = `eastus2`,api version = `2024-12-01-preview`,6 deployments(`gpt-5.5` / `gpt-5.4` / `gpt-5.4-mini` / `gpt-5.4-nano` / `text-embedding-3-small` / `text-embedding-3-large`,exact name 用 dot 而非 dash),api key 由 plaintext markdown 遷移至 root `.env`(H5 remediation commit `09138d4`)。**Note**:`gpt-5.5-pro`(per CLAUDE.md §5.2 H2 alternative eval judge)未 deploy POC stage,eval judge default 用 `gpt-5.4-mini`(spec-compliant)。**Outstanding minor**:per-deployment quota TPM(operational planning only,non-blocking)。<br><br>**W11 D1 2026-06-09 operational follow-up — pricing rate baseline gate**:W10 D3 F5.2 cost dashboard real-time wire surfaced pricing rate sub-decision per W11 prep deck §6.1。**Stakeholder decision Option B**(Karpathy §1.2 simplicity-first):placeholder rates preserved(`backend/observability/realtime_cost.py::_PRICING_TABLE` label `placeholder_publicly_quoted_rates_2026-Q2`)+ `cost_spike` rule × 1.5x ceiling preserved(`backend/observability/alerts.py` existing rule;rolling 7-day avg comparison serves anomaly detection intent)+ 7-day re-baseline post real cohort traffic W11+ scheduled。Option A NOT CHOSEN(spec-completeness ROI 不及 Beta timeline pressure)。Q4 status remains `Resolved (full)`;pricing rate accuracy non-blocking via spend cap proxy until W11+ re-baseline calibration。 |
| **Decided By** | Chris(acting as Stakeholder per 2026-04-30 session;detail delivered 2026-05-01;W11 D1 pricing rate Option B authorized 2026-06-09) |
| **Date** | 2026-04-30(initial)/ 2026-05-01(detail)/ 2026-06-09(W11 D1 pricing rate Option B sub-decision) |
| **Status** | `Resolved` (full) — pricing rate operational follow-up via Option B spend cap proxy(non-blocking) |
...
| **Decision** | **Approved — promote Multi-language(en/zh UI)入 Tier 1**(用戶 as Stakeholder scope + Chris 架構,2026-07-14)。**APAC driver = 內部 + 外部 both**:①內部 APAC 團隊(RAPO 香港 / 區域使用者)習慣 zh 介面 → adoption / 易用性;②對外 APAC 客戶 / 市場 zh 需求 → 對外 business。**Scope**:UI chrome 雙語 en/zh(非 RAG content 翻譯、非 JP)。**Implication**:both driver → zh 翻譯品質要求對齊**對外標準**(專業校對,非純內部粗譯),優先級最高。細時程 / 預期使用者數留 phase plan 細化。 |
| **Decided By** | 用戶(as Stakeholder scope + Chris 架構,per decision-form User-as-decision-owner pattern — 同 Q7 / Q9 / Q22 same-session authorization)|
| **Date** | 2026-07-14 |
| **Status** | `Resolved`(2026-07-14 — 雙半 approve:scope + 架構)。**下一步 = 開 i18n multi-day phase plan(§10 R1)**;architecture.md §11 + CLAUDE.md §5.4 H4 的 Tier 邊界 amendment(Multi-language en/zh 由 Tier 2 移出)為 phase 首個 deliverable;implement code 前仍需 phase plan committed。 |
...
| Q23 | Multi-language(en/zh)Tier 2→Tier 1 promote(NEW 2026-07-13 per ADR-0075 / B-25) | Stakeholder(scope)+ Chris(架構) | — (non-critical;post-W101 scope) | frontend(i18n NEW) | `Resolved`(2026-07-14 — 雙半 approve;APAC driver = 內部+外部 both;下一步開 i18n phase plan + Tier 邊界 amendment)| 2026-07-14 |
...
**OQ status snapshot 2026-07-14**(Q23 Resolved):**18 / 23 Resolved**(Q23 加入 — Multi-language promote **approved 雙半**,APAC driver = 內部 + 外部 both)+ **5 / 23 Open**(Q6 / Q8 / Q15 / Q16 / Q20 不變)。**下一步 = i18n multi-day phase plan(§10 R1)**;Tier 邊界 amendment(architecture.md §11 Multi-language 移出 Tier 2 + CLAUDE.md §5.4 H4)為 phase 首個 deliverable,implement code 前需 plan committed。ADR-0075 → Accepted。
