...
另有 1 個收尾細項(DD-5;**DD-3 + DD-4 已於 2026-06-11 close**)+ W59 過程新發現的 1 項
...
| **DD-7** | **Mega-section synth timeout**(W59 F4 新增) | 圖密 mega query(單 section 數十圖,如 AR S04=44 圖;Q001/Q036 預期 65 圖)synth 負擔超 production 30s `synthesizer_request_timeout_s` → `APITimeoutError`;W59 量度時用 `SYNTHESIZER_REQUEST_TIMEOUT_S=180` 繞過,production 30s 路徑未解 | 評估 production timeout 上調 vs mega-section context 縮減(注意:DD-4 / per-doc 放寬 returned cap 會**加重** synth context → 加重此風險,需一併權衡) |
...
- **提議 ③(per-config 對比)= 只做 single production default baseline**(A/B vs DD-4 未做)。
...
**未盡部分(後續)**:prose 型第二份 GT + per-config A/B 對比;§4.6 結構性路線現可由實數重評
(指標已有,位置繼承天花板實測在 mean recall ~0.57,cap-bound query 是主要拖累)。
...
3. **圖片召回指標(§4.5)** —— ✅ **W59 部分完成(2026-06-11)**:指標 harness + AR pilot baseline
   9/9 已出數(image-recall 0.5715 / image-precision 0.9824),產品核心價值首次可量度;亦是 Gap B 與
   §4.6 結構性路線的共同判據。**未盡**:prose 型第二份 GT + per-config A/B 對比(詳 §4.5 末「未盡部分」)。
