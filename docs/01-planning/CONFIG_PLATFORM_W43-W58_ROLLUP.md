...
**✅ 成功定義拍板(2026-06-12 用戶)**:**圖密文件成功 = 全圖召回**(先行,後可改)→ mega-query
0.62–0.74 缺口正式入 scope;GT 型指標軸鎖定 image-recall(將來入 UI 無返工風險)。

**✅ W66 落地(2026-06-12,phase `W66-cap-refs-vs-unique-diagnosis`,無 ADR)**:「48 plateau」真因
判決 — per-KB cap 臨時 50→150(跑完復原):refs 即刻脹滿任何 cap(60→150 全食,**cap 喺 refs 層
一直 binding**),但 **Q001 unique 釘死 48**(+90 refs 全部重複)/ Q043 unique +5(53)→
**48–53 = 收割範圍(cited chunks + 鄰居)unique 圖總量嘅真天花板**,漏網 GT 圖喺**未被 cite 嘅
chunk 區域**(W62 已證 window=8 無效 = 唔係 reach 半徑問題)。**dedup-before-cap 提案降級**(對
recall 邊際 +0~+5,唔值得單為 recall 行 H1;留 payload 衛生理由)。全圖召回剩餘路徑(全部 H1,
用戶決策):(a) 章節/程序範圍圖片收割機制(ADR-0047 pin 推廣版)/(b) §4.6 caption /(c) 接受
~0.74 為 Tier 1 旋鈕天花板。建議下一步 = 零 code 嘅漏網圖 chunk-level 映射診斷。

**✅ W67 落地(2026-06-12,phase `W67-missing-image-chunk-mapping`,無 ADR)— ⚠️ W66 判決推翻**:
漏網 17 張 chunk-level 映射(W59 catalog + cap=150 capture join):**17/17 嘅 owning chunk 全部被
cite**(26 citations 覆蓋 ci 5–30 = 成個 AR01 章節 → 供給 + reach 完整,「供給天花板」唔存在);
真兇 = **cap 預算計 refs + 前置 citation 高重複 neighbour aux 食盡預算**(時間線:citation 1–4 食盡
150 refs(48+40+40+22,68% 重複),citation 5–26 共 22 條 refs_kept=0;行到第一張漏網需 cap≈360 =
抬 cap 死路)。**dedup-before-cap 由 W66 嘅「邊際項」升返做唯一槓桿**:預算計 unique 嘅話 cap=65–70
行勻全部 citation → Q001 65/65 全部 in reach(0.74 → 潛在 1.00)。三岔口 (a) 章節收割 / (b) caption
