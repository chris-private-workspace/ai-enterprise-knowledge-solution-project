...
**未盡部分(後續)**:**prose 型第二份 GT**(硬依賴用戶提供 prose 文件 + 親自標注 GT,延後)+ cap 放寬軸
A/B(候選);§4.6 結構性路線現可由實數重評(位置繼承天花板實測 mean recall ~0.57–0.61,cap-bound query
是主要拖累,且 DD-4 已解 section-覆蓋型 miss)。
...
3. **圖片召回指標(§4.5)** —— ✅ **W59 指標 + baseline + W60 A/B(2026-06-11)**:W59 建指標 harness +
   AR pilot baseline 9/9(image-recall 0.5715 / image-precision 0.9824),產品核心價值首次可量度;**W60
   補做 DD-4 軸真 A/B**(第一次 env override 無效已撤回 — 被 drive-images-1 per-KB config 鎖死;改 per-KB
   真 toggle 重做):**真結論 = DD-4 三旋鈕對圖片召回中性偏負**(A 臂 0.572 vs B 臂 0.545–0.570,差異喺
   variance band 內;parent_doc=true 增不穩定 + Q005 偶 returned 0)。「+3.7pp 解 Q005」係 variance 誤判已糾正;
   圖片召回真瓶頸仍 = cap 天花板 → 下一槓桿 = per-doc 放寬 returned cap。亦是 Gap B 與 §4.6 結構性路線的
   共同判據。**仍未盡**:prose 型第二份 GT(硬依賴用戶提供文件 + 親自標注,延後)+ cap 放寬軸 A/B(候選)
   (詳 §4.5 末「W60 A/B 落地」+「未盡部分」)。
