...
### F0 closeout — gate DEFER（用戶確認 2026-06-16）

| Hash | Subject | Checklist |
|---|---|---|
| `cdafc70` | W85 kickoff + plan | plan |
| `e4f2525` | F0.1 結構對比 | F0.1 |
| `567329d` | F0.3 差異化 chunker 診斷（PIVOT） | F0.3 |
| F0 closeout | plan closed + DD-13 + memory | closeout |

- **gate 判決 = DEFER**:缺口① 兩路（F0.1 路由 / F0.3 造 chunker）皆實證否決。記 `DEFERRED_REGISTER` **DD-13**。
- F1 不開（造差異化 chunker DEFERRED）;ADR-0066 未用（保留下一 available）。

### Retro

- **做得好**:診斷先行擋住幻影框架 —— F0.1（結構對比）+ F0.3（chunk dump）兩步純 diagnostic 零 production 改動,
  用實證否決一個看似合理的 vision 缺口,省下中等成本框架（orchestrator 改 + ingest preset + UI + re-index）。
- **關鍵洞察**:用戶 vision「不同文件不同處理流程」在 ingest-chunking 層**已被通用 `layout_aware` 隱性滿足**
  （table row group + heading 邊界 + image-grouped via doc_order）;profile 差異要嘛被它吸收,要嘛被他層處理
  （圖密 → W59-83）。**vision 缺口未必是缺失,可能已被現有層滿足 —— 先 dump 實際 artifact 驗證再判**（W83 教訓再現）。
- **PIVOT 的價值**:用戶揀 pivot（造 chunker）而非直接 defer,讓 F0.3 把「為何不需要」查清楚（dump 實際 chunk 證明
  layout_aware 已合理）,比 F0.1 後直接 defer 更扎實 —— pivot 不是繞路,是把負面結論做實。
- **坑**:離線 script import 要 `-m scripts.x`（非 `python scripts/x.py`）;console cp1252 → 自引入非 ASCII 字元
  （`⏎`）會炸,輸出用 ASCII + `PYTHONIOENCODING=utf-8`;commit message here-string 內雙引號破壞 PowerShell 解析。
- **無 scope silent drift**:PIVOT 記 plan §7 changelog（R3）;F0.2 取消明標;DEFERRED 項標 🚧 + DD-13。

### 下一步（rolling JIT，等用戶 trigger）

- 缺口① defer（DD-13）。vision 剩餘真正缺口（per W84 後 status review）= 缺口②（各 profile preset 中值未實證,
  待真實多類型 KB 校準）+ 真實多類型 / scan KB 檢驗。層 B/C 仍 defer（DD-10/DD-12）。
