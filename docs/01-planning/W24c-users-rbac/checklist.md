...
- [x] **F9.4** Audit tab + role-gating + verify(R6 Day 12,10 findings)— **(a)** EDIT `lib/api/admin.ts` `AuditAction` Literal 5→10(mirror backend `audit_log.py` — 加 W24c F4/F7 RBAC actions,stale-type correctness fix);**(b)** `page.tsx` 加 inline `AuditTab`(workspace audit feed,`useQuery` → `adminApi.listAuditLog()`;`auditActionIcon` 5-prefix branch verbatim per mockup;`payloadNote()` synthesize 第 3 行 from `payload` scalar entries per §13;`Filter`/`Export` button inert per D11.4)per mockup lines 324-377;**(c)** `useRole()` whole-page role-gating wired(`/users` = Workspace-Admin-only per ADR-0027 matrix — `role===null` loading / `role!=='admin'` access-denied `banner-warning` / `admin` full page;shell query `enabled: role==='admin'`);**(d)** remove `TabPlaceholder`(全部 tab 用完);**(e)** NEW `tests/unit/users-page.test.tsx`(4-tab render + `?tab=` deep-link + role-gating admin/non-admin + per-tab smoke)+ EDIT `tests/e2e/app-shell-path.spec.ts`(+`/users` render-smoke)。H7 7-item self-verify PASS 全 4 tab(Members/Roles/Groups/Audit);tsc/lint/`[oklch`=0 + Vitest green;Playwright runtime = smoke-user-deferred per plan §3
