...
- /kb/[id] 7-tab `-Access` → ADR-0025 8-tab + ADR-0027 Access tab Wave C1(W22 ships 7-tab + disabled affordance on Access per CC10 H4 boundary;mockup `ekp-page-kb.jsx:140` PageKbDetail + sub-tabs in same file + `ekp-page-kb-extras.jsx` TabImages/TabChunkingLab + Wave C1 deferred `ekp-page-users.jsx:390` TabKbAccess)
- /kb/[id]/upload → ADR-0028 + Wave A 3-step rebuild preserved(mockup `ekp-page-misc.jsx:4` PageUploadWizard 3-step Data source / Document processing / Execute)
- /kb/[id]/docs/[docId] → ADR-0029 Option C 3-pane(W21 F3 deferred → W22 KB cluster;mockup `ekp-page-doc-detail.jsx:6` PageDocDetail — only 2 separate functions `ImageThumb` + `ChunkInspector`,其餘 inline per single-file pattern)
...
- **Spec ref**(per D8.a file-path correction 2026-05-18 D4):`references/design-mockups/ekp-page-kb.jsx:140 PageKbDetail` + sub-tabs `TabDocuments`(238-329)/ `TabChunks`(332-394)/ `TabPipeline`(689-742)/ `TabRetrievalTesting`(397-569)/ `TabKbSettings`(745-878)inline + `ekp-page-kb-extras.jsx:4 TabImages` + `ekp-page-kb-extras.jsx:257 TabChunkingLab` consumed by PageKbDetail + Wave C1 deferred `ekp-page-users.jsx:390 TabKbAccess`(disabled affordance Wave A);`ekp-page-misc.jsx:4 PageUploadWizard`(3-step Data source / Document processing / Execute);`ekp-page-doc-detail.jsx:6 PageDocDetail`(only 2 separate functions `ImageThumb` + `ChunkInspector` per single-file pattern)— per ADR-0025 8-tab + ADR-0028 + ADR-0029 Option C
- **OQ deps**:none(per D8.b 2026-05-18 D4 — F6.4 over-engineered framing dropped;mockup `ChunkInspector` lines 343-353 already uses synthetic 24-dim hardcoded float preview;render mockup-style synthetic visual;real Azure Search vector field exposure stays Tier 2 but user-invisible per CSS-first pivot)
- **Acceptance criteria**:
  - F6.1 Rebuild `frontend/app/(app)/kb/[id]/page.tsx` 對齊 `ekp-page-kb.jsx:140 PageKbDetail` 7-tab `-Access`(Documents + Chunks + Images + Chunking Lab + Pipeline + Retrieval Testing + Settings;Access tab disabled affordance — Wave C1 activates per ADR-0027 + CC10 H4 boundary);each tab inline within `page.tsx` per mockup single-file pattern(`DocumentsTab` / `ChunksTab` / `ImagesTab` / `ChunkingLabTab` / `PipelineTab` / `RetrievalTab` / `SettingsTab`)— mockup spec spread across `ekp-page-kb.jsx`(TabDocuments 238-329 / TabChunks 332-394 / TabRetrievalTesting 397-569 / TabPipeline 689-742 / TabKbSettings 745-878)+ `ekp-page-kb-extras.jsx`(TabImages 4-98 / TabChunkingLab 257-350);preserve backend hooks(`kbApi.get` / `kbApi.listImages` / `kbApi.chunkingPreview` / `kbApi.patchSettings` / `kbApi.patchMetadata` / `kbApi.archive` + `streamQuery` for RetrievalTab + `kbDocApi` for Documents+Chunks);use mockup CSS classes per CSS-first pivot baseline
  - F6.2 Rebuild `frontend/app/(app)/kb/[id]/upload/page.tsx` 對齊 `ekp-page-misc.jsx:4 PageUploadWizard` 3-step(Data source / Document processing / Execute);inline stepper per F5.3 defer decision(28px circle + letterSpacing -0.005em label + transition 0.2s + divider margin 0 4px per W22 D2 audit);**PRESERVE `kbApi.uploadDoc` mutation**(per D8.d — mockup Step 1 has drag-drop file picker;F5b /kb/new file-picker drop decision 唔適用 /kb/[id]/upload)+ existing `useState` step state + `useParams` + `useRouter` + `useQueryClient` invalidation
  - F6.3 NEW route + rebuild `frontend/app/(app)/kb/[id]/docs/[docId]/page.tsx` 對齊 `ekp-page-doc-detail.jsx:6 PageDocDetail`(W21 F3 deferred here);page decomposition:header + 5-stage pipeline strip(Parse / Extract / Chunk / Embed / Index)+ image strip(horizontal scroll of inline `ImageThumb`)+ 3-pane(outline 240px / chunk list 1fr / inspector 380px)— **all sections inline within `PageDocDetail` page.tsx per single-file pattern;ONLY `ImageThumb` + `ChunkInspector` extracted as separate functions** per mockup pattern(per D8.c — drop W21-era `<DocumentOutline>` + `<ChunkList>` + `<ImageStripScroller>` abstraction)。Consume W21 F1 backend `GET /kb/{kb_id}/docs/{doc_id}` enriched endpoint shipped(`306dbe0`)
  - F6.4 Embedding vector preview = mockup synthetic 24-dim hardcoded float preview(per D8.b — `ChunkInspector` lines 343-353:`border-1 oklch(var(--border))` + `background oklch(var(--muted)/0.4)` + 8-col grid + font-mono 10.5 + positive→`oklch(var(--accent))`/negative→`oklch(var(--foreground))`+ `…+1000 more dims…` footer);NO DisabledAffordance(real Azure Search vector exposure stays Tier 2 but user-invisible);card footer `embed-3-large · MRL truncate 1024d` + `Full JSON →` CTA(visual only Wave C+)
  - F6.5 Preserve all backend integration:`GET /kb/[id]`(KbStatus full)+ 7-tab existing endpoints(documents listing + chunks listing per `kbDocApi` + images per `kbApi.listImages` + chunking-preview per `kbApi.chunkingPreview` + retrieval-test per `streamQuery` + settings PATCH per `kbApi.patchSettings`+`patchMetadata` + archive per `kbApi.archive`)+ W21 F1 doc-detail endpoint per ADR-0029
  - F6.6 Tokens 100%(`Grep '\[oklch'` across `frontend/` = 0 preserved through F6 rebuild — mockup raw `oklch(var(--foo))` substrings must route via tokens.ts);`tsc --noEmit` exit 0;`next lint` clean
  - F6.7 CLAUDE.md §3.2.1 H7 fidelity 7-item self-verify + user-eye side-by-side verify per sub-page(3 sub-pages = 3 verifies — /kb/[id] + /kb/[id]/upload + /kb/[id]/docs/[docId])
...
- **Spec ref**:`ekp-page-eval.jsx PageEval`(301 lines)+ `ekp-page-trace.jsx PageTrace`(detail line 5)+ `ekp-page-trace.jsx PageTracesList`(list line 410)+ W17 F3 RAGAs 4-metric integration + W21 F2 GET /traces backend shipped `55f876b`(per D9.a/b file-path + function-name correction;both PageTrace + PageTracesList live in same singular-file mockup per `ekp-page-kb.jsx` precedent housing PageKbList + PageKbDetail)
...
- **Acceptance criteria**:
  - F7.1 Rebuild `frontend/app/(app)/eval/page.tsx`(648→TBD lines complete rewrite)對齊 `ekp-page-eval.jsx PageEval` 6 sections:**top stat-grid 4-metric**(MetricCard×4 — backend `EvalReport` labels R@5 / FFul / CRct / IAss per D9.d backend-wins override + visual polish-only migrate per W20 F7.2 precedent;mockup label drift Recall@5/Faithfulness/Ans Relevancy/Ctx Precision = pre-W17 RAGAs snapshot)+ **2-col grid 1.6fr/1fr below**:left col = RerankerShootoutCard(consume `evalApi.shootout` + `ShootoutReport.rerankers[].skipped`/`baseline`/`locked` badges)+ FailedQueriesCard(consume `report.failed_queries[]`);right col = RecommendationCard(text per ADR-0012 Cohere v4.0-pro production lock)+ OpsMetricsCard(`p95_latency_ms` + `avg_cost_per_query_usd` + Context recall row `<DisabledAffordance>` Wave C+ per D9.e + F7.7)+ CragInsightCard(`crag_trigger_rate` + 0.70 threshold note);page-header 3 actions(Run eval / Export / Reranker shootout);preserve W17 F3 `POST /eval/run` + `POST /eval/shootout` backend integration unchanged
  - F7.2 Rebuild `frontend/app/(app)/traces/page.tsx`(55→TBD lines complete rewrite)對齊 `ekp-page-trace.jsx:410 PageTracesList` 9-col table view;consume W21 F2 backend `GET /traces?filter=...&since=...&kb_id=...&limit=&offset=` shipped `55f876b` per `backend/api/routes/debug.py:42`;page-header 3 actions(Filter / Export / Open Langfuse ↗)+ search-wrap + **4-button seg toggle(All / Success / Error / CRAG triggered)mockup-faithful per D6 H7 fidelity correction 2026-05-18** — backend `?filter=` only supports 3 values(`all|errors|crag_triggered`),Success = client-side post-filter on top of `?filter=all`(`status === 'ok' && crag_iterations === null`),visual fidelity wins per W20 F7.2 precedent + H7;showing-N-of-total reflects filtered count for Success mode;Last X dropdown(map to `since` param);9 columns = Trace / **User(placeholder "—" per D9.f backend-wins;`TraceSummary` 不 expose user field)** / KB(`kb_id`)/ Query(`query_preview`)/ CRAG(`crag_iterations !== null` → badge `crag_iterations× loop`)/ Latency(`duration_ms`)/ Cost(`cost_usd`)/ When(relative from `timestamp`)/ chev-shrink;row click → `router.push('/traces/{trace_id}')`
  - F7.3 Rebuild `frontend/app/(app)/traces/[traceId]/page.tsx`(533→TBD lines complete rewrite)對齊 `ekp-page-trace.jsx:5 PageTrace` 3 viz modes(per D9.b function name correction `PageTrace` not `PageTraceDetail`);**ALL 7 viz functions inline within page.tsx**(per D9.c + Karpathy §1.2 + D8.c precedent):`TraceHeader`(80-116)+ `TraceVertical`(119-131)+ `StageRow`(133-212)+ `renderValue`(214-238)+ `TraceWaterfall`(241-299)+ `TraceFlame`(302-374)+ `FinalResponseCard`(376-407);viz mode default `vertical` persisted to `localStorage['ekp-trace-viz-mode']` w/ SSR-safe useEffect read pattern(per W15 D3 conversation-history localStorage precedent;mockup `tweaks.traceViz` is mockup framework convention not Next.js production)+ viz mode `<seg>` toggle 3 buttons(Vertical/Waterfall/Flame);top 5-stat strip(Total latency / Tokens / Cost / CRAG iterations / Status);TraceHeader breadcrumb back to /traces;preserve existing `debugApi.getTrace` backend integration(`GET /debug/trace/{trace_id}`)unchanged
  - F7.4 **ONLY** NEW typed client `frontend/lib/api/traces.ts`(per D9.g — `eval.ts` already exists W15 F1 + CH-002 F3 + W17 F3 RAGAs);schema mirror `TraceSummary` + `TraceListResponse` per `backend/api/schemas/observability.py:124-167`(W21 F2.1);API method `tracesApi.list({ filter, since, kb_id, limit, offset }): Promise<TraceListResponse>`
  - F7.5 ~~NEW viz components~~ **DROPPED per D9.c**(violation of mockup single-file pattern);replaced w/ in-page inline functions per F7.3 acceptance criteria above。No `frontend/components/traces/` directory created
...
| **2026-05-18 D2** | AI per user directive | **F5b /kb/new rebuild landed** + **Rule-of-3 wizard primitive promotion DEFERRED W23+**。F5.3 conditional verify per CLAUDE.md §5.7 H7 + Karpathy §1.2:mockup audit reveals **only 2 wizards use stepper UI**(`/kb/new` 5-step + `/kb/[id]/upload` 3-step;both 99% identical with 3 minor variations — letterSpacing -0.005em / circle transition 0.2s / divider margin 0 4px);`/register` 2-step + verify-email use **view-switching NOT stepper bar**;Rule-of-3 threshold(3+ instances)未達;defer primitive extraction per Karpathy §1.2「3 similar lines is better than premature abstraction」+ H7「mockup wins,不可 forced uniformity」。**F5b scope change**(per H7 — mockup wins):the W20-era first-doc file picker DROPPED from `/kb/new` wizard;mockup `PageKbNew` provisions empty KB only;document ingestion stays at `/kb/[id]/upload` `PageUploadWizard`(F6.2 scope)。Result:`/kb/new` redirects to `/kb/[id]` after create → user adds docs via upload wizard。Backend integration preserved:`kbApi.create` + KbConfig 9 fields + `useRouter` + `useQueryClient` invalidation;file picker + `kbApi.uploadDoc` + step-1-file-state removed cleanly。**Step reordering**(per mockup canonical sequence):Source → Parsing → Chunking → Multimodal → Review **→** Identity → Format & chunking → Multimodal → Retrieval defaults → Review;each step component rewritten with mockup CSS classes(`.card / .card-header / .card-body / .card-footer / .field / .input / .label / .hint / .seg / .seg-btn / .switch / .badge / .banner / .nav-section-label / .col / .row / .mono / .muted / .hr`)+ inline `style={{}}` for one-off mockup specifics(28px stepper circles + 5-col multimodal pipeline grid + Tier 2 colored card borders);Tier 2 preview fields(captioning_model / low_value_threshold / render_pdf_pages)preserved as UI-only state(never sent to backend per CC10 H4 boundary)。 |
| **2026-05-18 D8** | AI per user directive(F7.10 user-eye verify unblocked via backend Langfuse clamp fix + F7 phase-gate close)| **F7.10 user-eye verify final route `/traces/[traceId]` pass landed `(this commit)`** — 3 of 3 routes verified pass(D6 /traces + D7 /eval + D8 /traces/[traceId]);F7 phase-gate **CLOSED**;F8 unblocked for closeout cascade。**D8 backend regression fix landed inclusive `(this commit)`** — `backend/observability/langfuse_trace_list.py:47` `_DEFAULT_FETCH_LIMIT` 由 500 → 100 clamp;原因:W21 F2 commit `55f876b`(2026-05-17)時 Langfuse SDK 或 self-hosted dev environment 未 enforce 100-per-page cap,而家 cloud + newer SDK / self-hosted v2 enforce 咗;Frontend `tracesApi.list({ limit: 50 })` → backend `fetch_window = min(500, 50+0+100) = 150` → Langfuse 400「Number must be less than or equal to 100」surfacing as `status='fetch_failed'`。Fix scope:2 files(`langfuse_trace_list.py` 主要 constant change + 6-line comment + `debug.py` route docstring stale-500 reference update);per CLAUDE.md §5.1 H1 explicit exemption「Bug fix 可以自行做」;CC8 backend pytest verified pass(30 tests across `test_traces_route.py` + `test_debug_trace.py` + `test_langfuse_tracer.py`);backend restart 跟 W22 F7.10 verify workflow(killed stale PID 37036 started 2026-05-16 via system Python without `--reload`;new PID 2092 via `.venv/Scripts/python.exe` after intermediate stuck process cleanup)。**Process meta — D8 屬不同類 anti-pattern**:相對 D1/D6/D7/D9 4 條 cumulative **frontend fidelity** empirical-finding pattern,D8 係 **backend regression caught mid-frontend-phase** — separate category。CLAUDE.md §10 R5 explicit:「Phase closeout 之前任何 architectural-adjacent decision 必須寫 ADR」 — D8 唔屬 architectural change(per §5.1 H1 bug fix exemption),所以無需 ADR;但 memory `feedback_*` 可以 append 新 category catalog entry for「backend regression discovered during frontend verify chain」F8 closeout cascade considers。**Anti-pattern naming for catalog**:「assumed-permissive vendor SDK cap that vendor later tightened」— W21 F2 had reasonable assumption(self-hosted dev permissive,500 safe upper bound)but vendor tightened cap post-commit;mitigation = always grep vendor docs for hard limits during dependency-touching code AND test against production-shape backend during PR review。 |
| **2026-05-18 D7** | AI per user directive(F7 user-eye pre-commit audit catches eval-set picker addition)| **F7.1 /eval `evalSetId` picker removed from page-actions landed `(this commit)`** — user-eye `/eval` 並排 screenshot audit caught **before commit** that `eval-set-v0 30q` `<select>` element 顯示喺 `Run eval suite` 左邊,但 mockup `ekp-page-eval.jsx:19-23` page-actions **只有 3 個 button**(Run eval suite / Export report / Reranker shootout)— **冇 select**。Previous F7.1 implementation 「preserve pre-W22 `evalSetId` reactive state + add select 入 page-actions」過度 preserve pre-W22 UI element 而 mockup 唔存在嗰個 element。Per H7 strict reading + W22 D1 W22 F4 ChatHeader precedent(「inherited W20 surface not in mockup」)→ **hardcode `EVAL_SET_ID = 'eval-set-v0'`** + `EVAL_SET_SIZE = 30` const at module scope;remove `[evalSetId, setEvalSetId]` state + remove select element;**eval-set switch surface deferred to Wave C+**(when v1 lands per CO_W15_F1 Q14 SME labels,surface picker in Settings tab 或 env var,**唔係** page-actions)。**Plan + checklist F7.1 + progress.md Day 5 deviations 表 patched** w/ D7 H7 correction row。**Anti-pattern catalogued for `feedback_design_fidelity.md` memory empirical-finding append at W22 F8 closeout cascade — 5th cumulative empirical-finding pattern** after D1(F4 ChatHeader inherited W20 surface)/ D8(F6 KB cluster W21-era plan-text contamination)/ D9(F7 pre-active-flip mockup-vs-plan-vs-codebase mismatch)/ D6(F7.2 over-extending §13 backend-wins to drop visual element);**D7 = same-category sub-pattern as D1 differ surface**:「preserve pre-W22 UI element that's NOT in mockup」= H7 violation,proper response = drop the element + log scope deferral。**Process meta — emerging anti-pattern category**:「preserve pre-W22 UI element that mockup doesn't have」(D1 ChatHeader + D7 evalSetId select 兩條 evidence)deserves explicit anti-pattern naming喺 memory append for cumulative recall — current default behavior「preserve W18 baseline」(per W22 plan §0 Preserve list)需要 cross-check「mockup 有冇對應 visual element」before preserve decision;若 mockup 唔有 → drop + log as scope deferral,not「preserve for backward compatibility」。 |
| **2026-05-18 D6** | AI per user directive(F7 user-eye pre-commit audit catches Success button drop)| **F7.2 /traces 4-button seg restored landed `(this commit)`** — user-eye audit caught **before commit** that「Success」button(status-dot ready + label)dropped during F7.2 D5 implementation。Previous decision「drop Success per §13 backend-wins because backend `?filter=` only supports 3 values」was over-extending §13 scope — **§13 covers data contract conflicts(mockup expects field X that backend doesn't return → backend wins on field shape),NOT visual element removal(mockup has visual button N that backend filter mode doesn't have → drop button)**。Per H7 strict reading + W20 F7.2 visual-polish-only migrate precedent → restore 4-button seg + add Success as client-side post-filter on top of backend `?filter=all`(`status === 'ok' && crag_iterations === null`);Showing-N-of-total reflects filtered count for Success mode(visual fidelity wins over count-accuracy)。**Plan + checklist + progress.md deviations 表 patched** w/ REVERSED marker。**Anti-pattern logged for `feedback_design_fidelity.md` memory empirical-finding append at W22 F8 closeout cascade** — 4th cumulative empirical-finding pattern catalogued(D1 W22 F4 ChatHeader / D8 W22 F6 KB cluster / D9 W22 F7 pre-active-flip / **D6 W22 F7 over-extending §13 backend-wins to drop visual element**);D6 differs from D1/D8/D9 in being **post-implementation user-eye catch**(not pre-active-flip audit)— precedent for future「visual element drop」decisions = surface as H7 deviation + ask user not unilateral judgment call。**Process meta**:per D6 evidence,§13 backend-wins authority needs scoping clarification at W23+ CLAUDE.md governance pass — currently reads broadly as「architecture.md > design-mockups」but applicable scope = data contract conflicts only,not visual surface design decisions。 |
| **2026-05-18 D5** | AI per user directive(F6.9 user-eye verify pass + F7 kickoff pre-active-flip 5-step audit)| **F6.9 user-eye verify pass landed** — 3 sub-pages side-by-side(`localhost:3001/kb/{id}` + `/upload` + `/docs/{docId}` vs mockup hash routes)user verify accepted;checklist F6.9 flipped `[x]`;F6 cluster phase-gate closed,F7 observability cluster unblocked。**F7 kickoff pre-active-flip 5-step audit landed**(D1/D8/D9 process amendment recursive **3rd cumulative application** — `feedback_design_fidelity.md` empirical-finding section 將 append 第 3 條 mid-phase recursive catch evidence at W22 closeout cascade);mockup-vs-plan-vs-codebase grep 揭露 **9 條 D9 findings**:**(D9.a)** Plan + checklist reference `ekp-page-traces.jsx`(plural)— file 唔存在;actual = `ekp-page-trace.jsx`(singular)housing both `PageTrace`(detail line 5)+ `PageTracesList`(list line 410)per mockup single-file pattern(same convention as `ekp-page-kb.jsx` PageKbList+PageKbDetail);**(D9.b)** Plan F7.3 cites「`PageTraceDetail`」— actual function name `PageTrace`(no Detail suffix;window export `window.PageTrace`);**(D9.c)** Plan F7.5 NEW viz components extraction violates mockup single-file pattern — mockup PageTrace 內含 7 inline functions(TraceHeader / TraceVertical / StageRow / renderValue / TraceWaterfall / TraceFlame / FinalResponseCard)all within same file → DROP extraction per H7 + Karpathy §1.2 + D8.c precedent;**(D9.d)** Mockup MetricCard labels「Recall@5/Faithfulness/Ans Relevancy/Ctx Precision」vs backend `EvalReport` 4-metric「recall_at_5/faithfulness/correctness/image_association」 → backend-wins per §13(W2-era 4-metric labels R@5/FFul/CRct/IAss per existing eval/page.tsx W15 F1 Deviation #2 already established;mockup label drift = pre-W17 RAGAs snapshot);**(D9.e)** Mockup `ev.context_recall` + `ev.eval_set_id` + `ev.eval_set_size` + `ev.finished_at` 不 expose by `EvalReport` schema → `context_recall` 用 DisabledAffordance Wave C+ placeholder per F7.7;`eval_set_id` 用 page state(reactive `evalSetId`);`eval_set_size` = `failed_queries.length` + computed pass count proxy;`finished_at` = client-side timestamp 或 mutation success time(ShootoutReport DOES expose `eval_set_id`+`finished_at` — use those for shootout subtitle);**(D9.f)** Backend `TraceSummary` 不 expose `user` field — mockup 9-col table 第 2 column `User` → render placeholder "—" preserve 9-col mockup-faithful structure per visual polish-only migrate(W20 F7.2 precedent);**(D9.g)** Plan F7.4 NEW typed clients narrowed — `eval.ts` already exists W15 F1 + CH-002 F3 + W17 F3 RAGAs;**ONLY `traces.ts` NEW** consuming `GET /traces` per `backend/api/routes/debug.py:42`;**(D9.h)** Plan F7.3 viz mode persistence `localStorage['ekp-trace-viz-mode']` preserved(mockup `tweaks.traceViz` is mockup framework convention not Next.js production — preserve localStorage approach with SSR-safe useEffect read pattern per W15 D3 conversation-history localStorage precedent);**(D9.i)** Existing implementations fundamental decomposition drift vs mockup(/eval 648 lines W15 2-col vs mockup top-stat-grid + 1.6fr/1fr below;/traces 55 lines W18 thin baseline vs mockup 9-col table;/traces/[traceId] 533 lines W18 9-stage Collapsible vs mockup TraceHeader+5-stat+3 viz+FinalResponse decomposition)→ **complete rebuild not patch** per H7 + W22 D0 precedent。**Plan text adjustments landed `(this commit)`**:**§1 ADR mapping** F7 row file path correction;**§2 F7.1** spec ref D9.d backend 4-metric label override + D9.e missing-field fallback + W20 F7.2 precedent cite;**§2 F7.2** spec ref D9.f User column placeholder per backend-wins + mockup-faithful 9-col preserved;**§2 F7.3** spec ref D9.b function name correction `PageTrace`(not Detail)+ D9.c **all 7 viz functions inline within page.tsx**(extraction dropped);**§2 F7.4** narrowed to ONLY `traces.ts` NEW(`eval.ts` already exists);**§2 F7.5** DROPPED entire section(replaced w/ inline pattern note);**checklist F7.1-F7.10** mirror plan §2 D9 patches。**Process meta**:D9 audit = D1 process amendment 5-step recursive **3rd cumulative application**(累計 hit:D1=W22 F4 ChatHeader 7 signals 2026-05-18 D1 / D8=W22 F6 KB cluster 5 signals 2026-05-18 D4 / D9=W22 F7 observability cluster 9 signals 2026-05-18 D5);user-explicit framing「我需要你以後每一次當你不清楚一些事情或者需求或者其他東西的時候,先進行思考然後問用戶嘅意見」+「假設我每次有任何要求...是否有任何需要修改或者刪除其他部份」(2026-05-18 in-session)→ recursive enforcement working as designed per CO_W14_process_grep_verify formalization;empirical-finding memory append to `feedback_design_fidelity.md` 預定 W22 closeout F8 cascade。 |
| **2026-05-18 D4** | AI per user directive(F1-F5b user-eye verify pass + F6 kickoff)| **F1.9 / F2.7 / F3.7 / F4.9 / F5.7 user-eye verify pass landed** — user 確認 W22 F1-F5b 顯示效果可以接受;checklist 5 items 由 `[ ]` flip 至 `[x]`(per W21 retro NO「smoke-user-deferred」allowance for fidelity itself)。**F6 kickoff pre-active-flip 5-step audit landed**(per D1 process amendment recursive 應用)— mockup-vs-plan-vs-codebase grep 揭露 5 條 W21-era residual F6 plan-text contamination(D8.a-D8.e):**(D8.a)** Plan §2 F6 + §1 ADR mapping + checklist F6 references `ekp-page-kb-detail.jsx` + `ekp-page-kb-upload.jsx` — both 唔存在;actual = `references/design-mockups/ekp-page-kb.jsx:140` PageKbDetail + `ekp-page-misc.jsx:4` PageUploadWizard;`ekp-page-kb-extras.jsx` contains TabImages + TabChunkingLab consumed by PageKbDetail;**TabKbAccess** 喺 `ekp-page-users.jsx:390`(Wave C1 scope per ADR-0027 RBAC backend dep);**(D8.b)** Plan F6.4 「if Azure Search vector exposure expensive → `<DisabledAffordance variant="p3-preview">` Tier 2 chip」over-engineered;mockup `ChunkInspector` lines 343-353 已用 **synthetic 24-dim hardcoded float preview**(8-col grid + footer `embed-3-large · MRL truncate 1024d` + `Full JSON →` CTA)→ per H7 mockup wins:render mockup synthetic preview;NO DisabledAffordance for embedding preview itself(real Azure Search vector field exposure 留 Tier 2 但對 user 透過 synthetic-preview-style 完全 invisible);**(D8.c)** Plan F6.5 NEW components list `<DocumentOutline>` + `<ChunkList>` + `<ChunkInspector>` + `<ImageStripScroller>` 係 W21-era abstraction;mockup `PageDocDetail` 只有 **2 個 separate functions**:`ImageThumb`(thumb card)+ `ChunkInspector`(right pane);其餘 outline + chunk list + image strip + pipeline strip + header **全部 inline within `PageDocDetail`** per mockup single-file pattern(D1 cross-page contamination signal cited explicit);**(D8.d)** F5b /kb/new file-picker drop decision **唔適用** F6.2 /kb/[id]/upload — mockup `PageUploadWizard` Step 1 (Data source) explicitly 有 drag-drop file picker;**preserve `kbApi.uploadDoc` mutation** + existing `useState` step state;只 swap presentation 3-step labels per mockup canonical sequence(Data source / Document processing / Execute);**(D8.e)** Plan F6.1 「7-tab `-Access`」per H4 Tier 2 boundary ✅ correct(mockup 8 tabs visible但 Access tab full activation = Wave C1 per ADR-0027 + H4 wins over H7 per §13 When-in-Doubt「Tier 1/Tier 2 邊界模糊 → Default to Tier 2 out of scope」);Wave A 已 ship 7-tab + Access disabled affordance,W22 preserve 呢個 H4 treatment 同時 rebuild 7 active tab presentation 100% 對齊 mockup `ekp-page-kb.jsx:140` PageKbDetail + sub-tab components。**Plan text adjustments landed `(this commit)`**:**§1 ADR mapping** 4 處 file path correction;**§2 F6.1 spec ref** 改 `ekp-page-kb.jsx:140 PageKbDetail` + cite sub-tab `TabDocuments` lines 238-329 / `TabChunks` lines 332-394 / `TabImages` from `ekp-page-kb-extras.jsx:4` / `TabChunkingLab` from `ekp-page-kb-extras.jsx:257` / `TabPipeline` lines 689-742 / `TabRetrievalTesting` lines 397-569 / `TabKbSettings` lines 745-878;**§2 F6.2 spec ref** 改 `ekp-page-misc.jsx:4 PageUploadWizard`(3-step Data source / Document processing / Execute;stepper 28px + letterSpacing -0.005em + transition 0.2s + divider margin 0 4px per W22 D2 audit);**§2 F6.3 spec ref** 改 `ekp-page-doc-detail.jsx:6 PageDocDetail`(只 2 separate functions `ImageThumb` + `ChunkInspector`;其餘 inline);**§2 F6.4** 改:render mockup synthetic 24-dim preview matching `ChunkInspector` lines 343-353;**§2 F6.5** 改:drop `<DocumentOutline>` + `<ChunkList>` + `<ImageStripScroller>` abstraction;adopt mockup inline pattern;only `ImageThumb` + `ChunkInspector` are separate(extracted)functions per mockup pattern;**checklist F6.1-F6.9** mirror plan §2 adjustments。**Process meta**:D8 audit 係 D1 process amendment 嘅 5-step recursive 第 2 次成功 catch(D1 hit 第 1 次 W22 F4 ChatHeader contamination 7 signals;D8 hit 第 2 次 F6 5 signals);recursive enforcement working as designed per CO_W14_process_grep_verify formalization。 |
