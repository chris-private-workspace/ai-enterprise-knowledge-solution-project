...
| 2026-06-13 | **F8 placement 指標修正**:相似度 proxy 對重複性 procedural corpus 結構性誤報(run 2 字面 25.43%,人工覆核 74/74 行誤報歸因三機制,零誤導錯配)→ 加 (1) driver 評分前剝答案 citation placeholder + raw answers 持久化;(2) **次序一致性訊號**(`order_consistency` core + `scripts/check_marker_order.py`)做真錯位指標 — 真局部調換 **0/249 = 0%**。AC4 判決以「人工覆核 + 次序一致性」為基礎(達標,建議 W71 go);proxy 字面數 + 誤報機制全文記 progress Day 2,W71 kickoff 用戶最終裁決 | proxy 字面數同 gate 意圖背離 — 唔靜靜重定義 gate,兩個數都報 + 補正交自動訊號(R3)|
