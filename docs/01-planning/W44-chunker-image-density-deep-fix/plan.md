...
> **Owner**:Chris(技術 Lead)。
> **狀態**:**`closed`**(2026-06-04 — Gate **PARTIAL→PASS**;F0-F5 全 done;圖洪 57→8 + recall/faith flat + correctness −2.28pp answer_relevancy noise + 三源證實 = no-regression)。
...
- **2026-06-03 F4 deviation(R3)**:單次 eval 撞 **eval-harness decay**(Cohere 連打 rate-limit 401 + eval-set-v1-draft Q14 SME-validation pending empty-GT + before-baseline reindex 走)→ RAGAs aggregate(recall 0.8848/faith 0.8583)唔可信。切法 D no-regression 改由**三源證實**(G1/G2 硬證 + pytest text bit-identical + 單/3 sanity query healthy)。**User pick 全 rigor 重建** → F4 擴展 gold-eval rigor sub-track(F4.4 Cohere throttle / F4.5 discover candidates / F4.6 SME GT[Chris block] / F4.7 cap=None before / F4.8 cap=8 after / F4.9 隔離對比),原 F4.1-F4.3 superseded。**跨多 session**,F4.6 需 Chris SME 人手。順帶:drive config `extract_embedded_images` false→true(修 W43 0 圖)+ drive 重建 287→369 chunks/0→1012 圖/max 8。
- **2026-06-03 F4.4 done**:eval-only Cohere rate-limit throttle/retry `backend/eval/throttle.py`(per-query throttle spacing + `AsyncRetrying` 401/429/TransportError longer-backoff;**唔掂 production `cohere.py` reranker**);runner + orchestrator 兩 loop wire;pytest 41 passed(commit `4b23494`)。
- **2026-06-03 F4.5 ground → R3 deviation(GT 類型)**:`discover_chunk_ids.py` W2-stale(`engine.retrieve` 缺 ADR-0018 `kb_id` → 會 crash)已修 + argparse 化(--eval-set/--kb-id/--out/--top-k)。**更核心**:Karpathy §1.1 + R6 recursive plan-text 自檢揭 **chunk_id GT 係 chunker-specific** — cap=None(F4.7)同 cap=8(F4.8)re-chunk 出唔同 chunk 邊界,單一 `acceptable_chunk_ids` 無法 valid score 兩 index,F4.9「同 GT 唯一變 cap」對 chunk_id GT 內在矛盾。**User pick 內容導向 GT(keyword + optional reference_answer)** → chunker-agnostic,兩 re-chunk 用同一 GT(keyword-mode recall + RAGAs context_recall)valid 比較 + 永久修 eval-set-v1-draft empty-GT(Q14)。checklist F4.5-F4.9 語義更新:F4.6 SME 改寫 `expected_answer_keywords`(非揀 chunk_id),G3 改 keyword-mode recall。discover script 輸出改 surface top-k chunk_text preview 輔助 SME 寫 keyword。
- **2026-06-04 F4.6 done**:Chris chat 互動逐模組複核 50 query → apply `eval-set-v1-draft.yaml`(46 keyword GT + validated:true + 4 reclassified OOS Q048/Q049/Q050/Q054)+ metadata `1.0-w44-f4.6-sme-validated`;`validate_eval_set.py` 修 W2-stale(接受 keyword-mode GT + expected_refusal⇒oos 不變量)→ OK passed。Q14 designated SME validation 首次執行(decision-form Q14 Notes 加 traceability)。commit `889688e`(pushed)。
- **2026-06-04 reboot infra recovery**:用戶重開機 → 本地 infra 全停。重建:Docker Desktop 更新後重啟、`docker compose up -d`(Postgres/Langfuse auto-restart;**azurite docker image perma-503 MCR** per ADR-0017 R8/R9 → native npm azurite Plan B `azurite --blobHost 0.0.0.0 ... --location infrastructure/azurite-data --skipApiVersionCheck`)。**關鍵教訓**:呢部機 post-reboot 重度 contention 下 azurite + backend **startup 慢到 ~30-90s / ~10min**(非 hang)—— 之前誤殺幾次皆因 ~12s 太早 check;耐心後皆 up。
- **2026-06-04 F4.7/F4.8/F4.9 done — Gate PARTIAL→PASS**:F4.7 cap=None before(recall 0.933/faith 0.9506/corr 0.795,46 eval/0 error);F4.8 cap=8 reindex(287→369 chunks force-split,curl multipart re-upload per doc-level reindex API 要求;Azurite 圖上傳正常)+ after(recall 0.9312/faith 0.9459/corr 0.7722);F4.9 隔離對比(同 SME GT 同條件唯一變 cap)recall −0.18pp ✅ / faith −0.47pp ✅ / corr −2.28pp ⚠️ marginal(answer_relevancy noise)→ **Chris 拍板 PARTIAL→PASS**。F5 closeout:architecture.md §3.3 ADR-0041 inline amendment + ADR-0041 Validation 段 + session-start §10 + roadmap §3 W44 done。**Phase closed**。
