...
- [x] F5.2 Progressive `@observe` decoration on query/synthesizer/crag stages — **W9 D1 done 2026-05-26 + W9 D2 LLM upgrade 2026-05-27** — W9 D1 baseline:`backend/observability/observe.py` NEW thin wrapper(`observe_async` decorator + `_emit_trace_safe` helper);3-stage decoration:`Synthesizer.synthesize` + `RetrievalEngine.retrieve` + `CragLoop.refine`;10 unit tests。**W9 D2 LLM upgrade**:NEW `observe_llm_async` decorator emits `client.generation()` for LLM-stage methods(model + usage{input/output tokens} mapping → Langfuse cost-attribution dashboard real-time USD);`_emit_generation_safe` helper graceful fallback to `trace()` when SDK lacks `generation()`;**H5 SECURITY**:wrapper passes ONLY `model` + `usage` + metadata,NEVER `input` / `output` text(verified via `test_llm_decorator_h5_no_prompt_or_answer_text_emitted`);synthesizer.synthesize 切換到 observe_llm_async;7 NEW unit tests = 17 total;**W9 D3+ scope** = apply observe_llm_async to `CragGrader.grade` + `CragGrader.rewrite_query`(grader / rewriter LLM cost attribution);329/329 pytest pass
