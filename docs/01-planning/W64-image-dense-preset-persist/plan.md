...
| 2026-06-12 | **Phase closed**(F1–F4 全 done)| **preset 已 persist**(drive-images-1:max_aux=40 / rerank_k=10 / cap=50,readback 為證);120s sanity **AC1 零 timeout + AC2 mean 0.855 / precision 0.988**(cap-bound 對齊 W62 band;Q005 0.69 = variance 注腳非 flip)。沿途兩次 infra 中斷(`APIConnectionError` transient + `getaddrinfo` DNS 跌)probe-恢復-重試處理,降級網絡數據作廢。零 code,無 ADR |
