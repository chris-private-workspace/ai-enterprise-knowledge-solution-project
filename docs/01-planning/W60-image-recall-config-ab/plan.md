...
| 2026-06-11 | **⚠️ 上行 closed 結論 INVALIDATED + 真重做** | 發現 env override global 被 drive-images-1 per-KB config(`false/10/1`)鎖死(`_resolve` per-KB 贏,query.py:144-145 讀 per-KB);兩臂 effective config identical → 「+3.7pp / 解 Q005」係 variance 誤判。用戶選「正確重做」:改 `PATCH /kb/{id}/settings` per-KB config 真 toggle(`false/2/0` vs `true/10/1`,跑完改回原值)。**真結論:DD-4 三旋鈕對 image-recall 中性偏負**(max_aux/depth 中性;parent_doc=true mean 0.545–0.570 ≈ A 臂 0.572 但增不穩定 + Q005 偶 returned 0);Q005=0 經 confirm run 證實係 fluke(回 0.28)。詳 progress.md「Day 1 (cont)」|
