...
- **✅ W49(本期,shipped 2026-06-05)— faithfulness 質素軸抗噪 N-run band(決策 7 Option 1)**
  ...
  - **採用 Option (a) N-run band**(下方候選做法)+ **(c) N=1 warning**;**(d) length-bias 標示** → ✅ **W50 shipped**(平 part:UI explainer affordance);**(d) completeness 對沖指標** → ✅ **W51 shipped**(ideal 半邊:**「涵蓋章節數」(distinct-sections-cited)per-run band** = coverage proxy 擺 faithfulness 旁,caveat 指向佢;誠實標明 **proxy 非 ground-truth recall**)。**synthetic-QA 真 recall** → ✅ **W52 shipped**(更深半邊:offline 工程閘 `backend/eval/synthetic_qa.py` — 由 KB chunk LLM 生成問題,source chunk = ground truth,餵現有 `EvalRunner` strict-mode 量 self-supervised Recall@K;**非人手 ground-truth recall**,bias disclosure 見 eval-methodology.md §10.6;**兩者合一 W52 基建** → **W53 ✅ shipped**:heading_aware 真 strategy(ADR-0044)+ `run_strategy_recall_comparison` 跨 chunk_strategy self-retrievability 比較 → **W54 ✅ shipped**:**controlled shared-question A/B**(`backend/eval/controlled_comparison.py` — 一個 frozen text-anchored[section_path strategy-invariant]question set 跨所有 strategy 跑 `EvalRunner` keyword-mode recall,消除 W53 per-config 問題集 confounding;**仍 synthetic + lexical-containment proxy 非人手 ground truth**,§10.6 標清))。
...
**修訂史**:
- **2026-06-07 CH-006 — per-KB「答案詳細度」config(concise | detailed)** —— W56 後續 chat 實測揭:procedural 問題(GL03 post journal)就算 recall 完整(parent_doc → 14 citations 含 Excel 分支)答案仍係 6 步摘要。根因 = synthesis prompt Rule 3「target <= 150 words」cap(**非 retrieval**)。用戶揀 **per-KB config 路線**(最貼合本願景)。走正式 **Change 流程**(spec approved → impl → test → live verify → closeout):`KbConfig.answer_detail`(C02,nullable inherit 模式)+ `Settings.synthesis_answer_detail` 全域預設 + `EffectiveConfig` resolve(per-query > per-KB > global)+ `prompt_builder.SYSTEM_PROMPT_DETAILED`(C05,= concise `.replace()` Rule 3 → 無字數上限 + 逐 sub-step;concise 逐字不變 = 零 regression)+ synthesizer / CRAG re-synth / query 兩路 thread `detail_level` + SettingsTab `.seg` 控件。**live-verified**(`w56-drive-ab-1`,真 chat pipeline):detailed → 3531 字完整 nested 逐 sub-step(同 manual);concise → 722 字摘要(W2 baseline 不變)。**體現本願景核心**:同一 KB 同一問題,per-KB config 一撥即切 synthesis 行為,無需改 retrieval。14 backend test + 2 frontend test 新增;0 新 regression(1 個 pre-existing citation-expansion 失敗,clean checkout 已存在,非本 CH);非 H1(無 §3/§4 component 增刪 / vendor swap)→ 無 ADR。commit `c460cd9`(F0)+ `6172173`(backend)+ `038a6a1`(frontend)+ closeout;CH folder `docs/03-implementation/changes/CH-006-per-kb-answer-detail-level/`。**遺留另案**:`default_rerank_k` → chat 嘅 wiring gap(chat 寫死唔送 top_k,KB 設定值無效)未修,待開獨立 CH/Bug。
- **2026-06-06 W56 — 大 corpus controlled A/B(recall 軸脫離飽和)+ W53/W52 CLI event-loop bug 修/verify** —— 收 W55 carry-over。Pre-flight **直接查 Azure**(非靠 metadata)= 1 live index/2 free(Free-tier 3-cap);`drive_user_manuals` metadata stale(index 已清 + 冇 `-sources`)不可 reuse → ingest fresh 6-doc KB `w56-drive-ab-1`(AR/AP/CB/FA/BM/GL,**images off**,**369 chunks** >> fetch_k=50;無 AP timeout)。**W54 `run_controlled_ab_comparison.py` 大 corpus**:layout_aware **0.9917**/369chunks、heading_aware **0.9850**/415chunks → **W55 saturation(1.0/1.0)打破**(兩個 <1.0 + delta),**但辨別力弱**(0.67pp gap 喺 n=30 synthetic-lexical noise floor → 不宣稱 winner);**穩健信號 = chunk-count 分化 369 vs 415(+12.5%)再證 ADR-0044**。**W53 `run_strategy_recall_comparison.py` event-loop bug verify-then-fix**:先跑 unfixed 確認 psycopg ProactorEventLoop crash @ lifespan startup → 套 win32 `loop_factory=SelectorEventLoop` guard(mirror W55)→ 重跑 exit 0;順帶 close W55 deferred self-retrievability cross-check(layout 0.9667/heading 0.9333)。**三角驗證(最高 signal)**:W53 confounded gap 3.3pp > W54 controlled gap 0.67pp = per-config confounding 放大 → 實證 W54 controlled 設計價值。**W52 `run_synthetic_recall.py`**:跑通 exit 0 無 crash → **不需修**(不用 lifespan/psycopg);3 個 smoke-deferred CLI 至此全部 live-exercised。**誠實**:強辨別 recall 需 human ground-truth(eval-set-v1)或更低 top-k。無 H1/ADR/vendor/dep(W53 改 driver guard 非 §3/§4);20 eval test passed 0 regression;3-index budget 全程守住(冇刪 index,2 used/1 free)。commit `dc69c58`(F0)+ `26495ee`(F1 ingest)+ `8ee1322`(F2 A/B)+ `191a1f8`(F3 W53 fix);phase folder `W56-large-corpus-ab-recall/`。W57+ 候選:eval-set-v1 human ground-truth recall / lower-top-k 實驗 / production default flip。
- **2026-06-06 W55 — W54 controlled A/B live-verified 對真 KB(smoke-deferred 實證)** —— 收 W54 carry-over。Pre-flight 揭現有 9 KB 全冇 `-sources`(W46 source-store 後置)→ ingest fresh KB `w54-live-ab-1`(CB Cash & Bank manual,28 chunks,sources 存到 + section_path 真階層)。跑 `scripts/run_controlled_ab_comparison.py` **exit 0 跑通**(11 text-anchored QA → reindex layout_aware[28] / heading_aware[33] → keyword-mode recall）。**修 2 個 CLI live-path bug**(從未 live 跑):(1) win32 ProactorEventLoop ✗ psycopg → `loop_factory=SelectorEventLoop`;(2) cp1252 console UnicodeEncodeError → print glyph 改 ASCII。**結果**:兩 strategy recall@5 都 **1.0**(飽和 — 小 corpus `fetch_k=50` >> ~30 chunks → 全 corpus 入池 → recall 觸頂,**corpus-size artifact 非 strategy 屬性**);**chunk 數分化** layout_aware 28 vs heading_aware **33(+18%)** → **live 證實 ADR-0044 heading_aware 結構性唔同**(no-merge 主導)。**誠實**:recall 軸喺單 mid-size doc 量唔到 strategy 差異 → 要大 corpus(`fetch_k < 總 chunk`)。W53/W52 CLI 同款 event-loop latent bug → carry-over。commit `b32b852`(F0)+ `aca7131`(F1-F2 live+fix);phase folder `W55-live-verify-strategy-ab/`。
- **2026-06-06 W54 shipped → controlled shared-question A/B(W53 self-retrievability 嚴謹版)** —— 收 W53 明文 deferred:用**一個 frozen question set 跨所有 strategy 評分**消除 W53 per-config 問題集 confounding。**R6 grep 揭三發現**(① `EvalRunner` 已內建 **keyword-mode recall**[`runner.py:166-184`,chunk_id 無關]→ reuse 零新 recall 數學;② `section_path` strategy-invariant[`HeadingAwareChunker` 繼承同一 section-walk]→ 做文字錨點;③ W53 dataclass self-retrievability framing 不可 reuse → W54 自有 controlled dataclass)。NEW `backend/eval/controlled_comparison.py`(C06):reference index → `_collect_chunks`(reuse W52)→ 按 `section_path` group section passages → judge LLM(gpt-5.4-mini)逐 passage 生**一問題 + 3-5 verbatim keywords** → `build_shared_eval_set` 寫一個 frozen keyword-mode eval-set → `run_controlled_strategy_comparison`(per strategy reindex → `EvalRunner` keyword mode 跑**同一 frozen set**)。CLI `scripts/run_controlled_ab_comparison.py`(smoke-deferred)。**誠實 framing(R1 — 誠實階梯第四期)**:**IS** controlled(消 confounding),**但仍** synthetic(LLM Q+keywords)+ **keyword-containment lexical proxy**(含詞 chunk ≠ 正確 answer-bearing chunk;偏寬鬆)→ 相對信號非絕對 verdict;§10.6 標清。**無 H1**(純 C06 eval extension,EvalRunner keyword mode reuse 非改)**無 ADR/vendor/dep**。廣 eval suite 82 passed = 0 regression(無改現有檔)。commit `dec9373`(F0)+ `dda0574`(F1 QA 生成)+ `5eb1baf`(F2 harness)+ `0d2a243`(F3-F4 CLI+tests);phase folder `W54-controlled-ab-recall-comparison/`。line 112 lineage 標 W54 shipped。W55+ 候選:人手標註 ground-truth recall / per-document scope。
- **2026-06-06 W53 shipped → 兩者合一下半截 done(chunk-strategy recall 比較)** —— 用 W52 synthetic recall 跨 `chunk_strategy` 比較。**R6 grep 揭三發現**(① chunk_strategy degenerate:auto/layout_aware/slide_based 全 LayoutAwareChunker + heading_aware 未實作;② reindex `_select_chunker` 只睇 image-cap IGNORE chunk_strategy;③ 跨 reindex chunk_id 變)→ Chris AskUserQuestion 揀**實作 heading_aware 真 strategy** + **per-config 重生 QA**(reuse W52)。**ADR-0044(H1)**:`HeadingAwareChunker`(thin LayoutAwareChunker subclass:target=hard_cap 取消 sub-split + min_merge=0 取消 merge → 更粗/少 chunk)+ wire chunk_strategy 入 `strategies.select_chunker` + `documents.py _select_chunker`(順帶 close W46 reindex over-promise gap)。`backend/eval/strategy_comparison.py` `run_strategy_recall_comparison`(per strategy reindex→synthetic recall→收集,零新 recall 數學)+ CLI `scripts/run_strategy_recall_comparison.py`(smoke-deferred)。**誠實 framing(R2)**:per-config 重生 QA = **self-retrievability 非 controlled A/B**(問題集逐 strategy 唔同),建基 W52 synthetic recall(非人手 ground truth);bias 見 eval-methodology.md §10.6。無新 vendor/dep。chunker/parser/kb_reindex 49 + kb_reindex/synthetic_qa/eval_runner/W53 34 passed = 0 regression。commit `b94bffc`(F1 ADR)+ `a9b635c`(F2 chunker+wiring)+ `32c8147`(F3-F4 harness+tests);phase folder `W53-chunk-strategy-recall-comparison/`。W54 候選:controlled shared-question A/B(text-anchored)。
