...
| 2026-06-10 (D3) | F3.6 auth flow wire deviation — defer ALL auth wire(含 existing MSAL SSO useAuthStore W7 baseline)to F5 batch | User instruction option A "F3 V8 Login UI(frontend-only,可獨立 work without F5 backend ready;auth flow wire 留 F5 lands)" — Karpathy §1.1 think-before-coding surfaced ambiguity:plan F3.6 原文 SSO 用 existing useAuthStore W7 vs user 嘅 strict 解讀「auth flow wire 留 F5 lands」涵蓋全部 auth wire。**Strict reading 採用** = UI shell only with stub click handlers + toast feedback(`F5_PENDING_MESSAGE` constants);F5 cascade 將 cleanly 連接 dual auth flow(MSAL SSO `useAuthStore.signIn()` + self-register `POST /auth/login`);loading + error UX 透過 local state 維持 visual polish | Chris(user instruction) |
| 2026-06-10 (D4) | F4.6 backend integration deviation + BrandPanel rule-of-2 extraction | (1)F4.6 wire defer 同 F3.6 pattern 一致 per user instruction option A "F4 V9 Register 3-step wizard(frontend-only,reuse V8 BrandPanel split + Stepper visual pattern + step transitions;backend wire 同樣 stub/F5 defer)";stub `handleStep1Submit` + `handleStep2Submit` + `handleResend` w/ `F5_PENDING_REGISTER` / `F5_PENDING_VERIFY` / `F5_PENDING_RESEND` constants + sonner toast feedback;TODO(W13 F5)comments mark replacement points;F4.7 error states scaffold via local validation only(client-side AccountInfo form rules)。(2)BrandPanel **rule-of-2 extraction** to `frontend/components/auth/brand-panel.tsx`(shared)— per design ref §2.9 explicit「Brand panel(left,same V8)」+ drift-prevention(V8/V9 panel updates would otherwise need 2 places);V8 Login refactored to import shared component(surgical 5-line touch only);Stepper inline kept(rule-of-3 pending — Pipeline wizard W12 + Register W13 = 2 active state-machine wizards;extract when 3rd emerges)| Chris(user instruction)|
| 2026-06-10 (D5) | F5 implementation deviations:(1)password hash lib swap argon2-cffi → hashlib.scrypt;(2)verification token format 32-char URL-safe → 6-digit numeric code;(3)session middleware as Depends extension(not separate ASGI middleware);(4)httpOnly cookie defer Beta | (1)**ADR-0016**:R8 Ricoh corp proxy SSL inspection blocked `pip install argon2-cffi`(empty hash IncompleteRead 0 bytes);user选 hashlib.scrypt(Python 3.12 stdlib;OWASP-approved memory-hard KDF;0 external dep);params N=2^17/r=8/p=1 + maxmem=256MB;storage format `scrypt$N$r$p$salt$hash` forward-compatible;**Beta hardening trigger**:revisit Argon2id if proxy resolved。(2)V9 wireframe §2.9 explicit「6-digit code」+ industry-standard OTP UX = backend stores 6-digit numeric code(`secrets.randbelow(1_000_000):06d`)not 32-char token(`secrets.token_urlsafe(32)`);entropy 20-bit acceptable Tier 1 with rate-limit + 60s cooldown + 24h expiry。(3)F5.6 plan said "session middleware";existing W7 pattern uses FastAPI `Depends(get_current_user)` not ASGI middleware;**implemented as session branch in `dependency.get_current_user`**(BEFORE mock/MSAL fork);proper middleware layer (`_PROTECTED_PREFIXES`)remained reserved for rate-limit/audit-log per W7。(4)F5.5 plan said httpOnly cookie + body token;implemented body token only(API-explicit pattern);cookie defer Beta hardening — get_current_user reads Authorization header per W7 baseline。(5)/auth/logout extended to revoke session token if bearer presented(non-breaking for mock/MSAL paths)| Chris(user instruction option A "F5 backend hybrid auth begin"  + ADR-0016 H2 vendor decision option 1 selection)|
