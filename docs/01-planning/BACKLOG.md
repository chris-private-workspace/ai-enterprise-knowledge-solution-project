...
| **B-25** | 雙語 i18n(en/zh UI 切換)— Multi-language Tier 2 promote 評估(建 dictionary + locale state + 全站 UI string externalize + 切換 UI + 新 i18n dependency 如 next-intl) | `ADR Accepted → 待開 phase` | **[ADR-0075](../adr/0075-multi-language-i18n-tier2-promote.md) Accepted(2026-07-14)** + decision-form Q23 Resolved:用戶 as Stakeholder scope + Chris 架構**雙 approve**,APAC driver = 內部+外部 both;scope 限 **UI chrome 雙語** en/zh(非 RAG content/JP)。**下一步 = 開 i18n multi-day phase plan(§10 R1)**,Tier 邊界 amendment(architecture.md §11 Multi-language 移出 Tier 2 + CLAUDE.md §5.4 H4)為 phase 首 deliverable;**implement code 前需 plan committed**。最大成本 = zh 翻譯(對外標準專業校對)+ 雙語同步維護 | [ADR-0075](../adr/0075-multi-language-i18n-tier2-promote.md) / architecture.md §11 / CLAUDE.md §5.4 H4 / [CH-023 §6](../03-implementation/changes/CH-023-unify-frontend-ui-language-english/spec.md) |
