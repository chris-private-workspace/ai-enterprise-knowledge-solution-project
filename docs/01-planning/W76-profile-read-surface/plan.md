...
- 2026-06-15 F1-F4 implement — schema + store + persist + expose + 14 test;regression 57 passed
  零 break;ruff + mypy --strict 新 code 0(commit `85963a5`)。
- 2026-06-15 **R3 deviation**(F4.2)— e2e ingest persist test 改為元件覆蓋(`from_result` +
  store upsert,已分別測);inline persist guard 邏輯 trivial,唔 mock Azure orchestrator
  (Embedder/IndexPopulator/Search mock 過重,Karpathy §1.2)。元件 + route-level join 已充分。
- 2026-06-15 **F5.1 verify**(策略 §3.4)— 採 unit/route-level(14 test 含 TestClient route-level
  list_documents/doc_detail 真實 join logic + schema serialization);**live-ingest end-to-end smoke
  DEFERRED**(infra:backend 8000 down + Azure cred 受限,對齊 EKP CO smoke-deferred 慣例)。現有 KB
  re-index 後驗自動 persist 留 infra 可用時 / 連段③ UI 一齊驗。
- 2026-06-15 **F5.2 R5 評估**:無新 ADR(persist + expose 屬 ADR-0056 層 A 既定範圍 —— D3「自動套
  preset 落該文件」+ D4 L2/L3「睇 profile + 信心度」隱含 profile 要 persisted + readable;新 store
  沿用 ADR-0023 Postgres + ADR-0050 store pattern,非新 architectural decision)。ADR-0056 正文不改
  (段落地記 memory + progress,同 W73/W74 慣例)。
- 2026-06-15 closeout — plan closed,F1-F5 全 tick,memory append。
