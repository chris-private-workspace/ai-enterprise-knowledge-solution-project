...
status: closed               # draft | active | closed — Phase Gate STRONG PASS 7/7 (2026-06-02); F0→F4 all done
...
| 2026-06-02 | **F4 closeout — Phase Gate STRONG PASS 7/7;status active → closed**。F4.1 pre-flight green;F4.3 G2 live 2-KB(AR 保守 3cit/8img 完整 + DCE inherit 5/5,全域零改)；F4.4 governance reuse ADR-0027 audit(無新 code)；F4.2 雙軸 no-regression(code-level byte-identical + RAGAs `eval-set-v1-draft ×30` recall 1.0/faith 0.9956/0 attention)。**F4.2 index-rebuild deviation(R3)**:drive eval 索引 `ekp-kb-drive-v1` 已被刪 → DELETE+POST /kb 重 provision + curl 上傳 6 docx 重建 287 chunks(精確對齊 baseline)+ backend orphaned-eval 卡死 → kill+restart venv(Postgres KB 完好)。W44+ candidate = chunker 圖洪水深層修 ADR-0041。 | F4 驗證執行 + drive eval 索引 infra decay | Chris(closeout)|
| 2026-06-02 | **F3.1 — KB Detail Settings tab design-stage expansion(改 mockup,design-first)**:F3.1「mockup 確認」發現 design-mockups **兩個 F3 surface 皆缺**(12 旋鈕 + 試跑面板)→ STOP+ask per §5.7 H7 → 用戶揀「改 mockup」。`references/design-mockups/ekp-page-kb.jsx` `TabKbSettings` 加 2 全寬卡(Advanced retrieval tuning 12 旋鈕分 3 組 + 進階收合 + 繼承/覆寫態;config-test 試跑面板 A/B + variance band)+ 4 helper。Playwright render 驗證 + 用戶批准。**非新 ADR**:ADR-0040(Accepted)已 cover config-scope,UI surface 為其自然呈現;PAGE_INVENTORY entry #5 同步。Visual fidelity 仍受 H7 約束(scope confirm 後)。 | F3.1 mockup 缺對應 spec(§5.7 H7 trigger)+ 用戶 design-first 決定 | Chris(批准設計)|
