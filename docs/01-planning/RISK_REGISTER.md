...
| **R-W26-1** ★ | **ADR-0037 parent-doc dispatch chain replace-vs-append architectural variable**(NEW W26 F2 D5 2026-05-25)| W26 F2 G RAGAs eval delta vs F1 baseline FAIL — faithfulness -8.36pp + correctness -6.12pp + Q-W25-I07 critical 0.00/0.00 + Q-W25-I01 control 被破壞 → root cause hypothesis 4-axis D1.35 含 dispatch chain replace 而非 append + LLM 見 parent_section_text + cites parent siblings outside top-5 reranked set | 🟡 **PARTIAL MITIGATED W27 D3 2026-05-25**(W27-parent-doc-dispatch-experiment F3 closeout per ADR-0038)| C05(prompt_builder + crag + query)+ C04(parent_doc_retriever)| 🟡 **Partially mitigated 2026-05-25** — W27 F2 G empirical proof:append mode **大幅修復** W26 F2 G replace 嘅 catastrophic regressions(faith +5.76pp / correctness +7.90pp / **Q-W25-I07 critical recovery 0.00 → PASS** — D1.35 H1 + H4 validated)but G1 + G4 marginal MISS by <1pp each vs F1 baseline tolerance ±2pp / ±0.05;Phase Gate PARTIAL per ADR-0038。**Settings default preserve `parent_doc_dispatch_mode="replace"`** per Q4 measurement-experiment-fail-policy + Karpathy §1.3 surgical 唔觸 revert。**W27 F1 infrastructure preserved** as W28+ enabler(Setting + branch + 4 NEW unit tests)。**W28+ candidates locked priority**(per ADR-0038 §Decision #4):**1 HIGHEST(b) `max_tokens_per_parent` 4000→2000/1500 sweep**(H2 attention dilution direct intervention — emerging primary residual axis;~3-5 days)**2 Second(c) RAGAs orchestrator-aware judge tune per R-W26-2**(~5-7 days)**3 Third(d) F3 query expansion standalone test per ADR-0034**(~3 days) |
| **R-W26-2** ★ | **RAGAs faithfulness judge misalignment with citation invariant when LLM cites parent siblings outside top-5**(NEW W26 F2 D5 2026-05-25)| 同 R-W26-1 — F2 G eval Q-W25-I07 critical 0.00/0.00 + aggregated faithfulness -8.36pp;hypothesis:**架構設計上 sound**(LLM 見 parent_section_text + citation 用 anchor chunk_id per architecture.md §3.5 Citation contract)**但 RAGAs judge 對 top-5 reranked chunks 評 faithfulness**,若 LLM 用 parent siblings outside top-5 generate statements → judge mark unfaithful;**metric mechanism mismatch** with architectural enhancement | 🟡 **PARTIAL DECAY W27 D3 2026-05-25**(per ADR-0038 D1.35 H1 validated)| C06 Eval Framework + C05 Generation | 🟡 **Decayed partially 2026-05-25** — W27 dispatch chain append mode 大幅 mitigate R-W26-2(append mode preserves anchor chunk_text in LLM input → LLM cite anchor chunk_id in top-5 → judge mismatch reduced)= W27 F2 G empirical evidence by Q-W25-I07 catastrophic 0.00/0.00 → PASS recovery + W27 faithfulness aggregate +5.76pp vs W26 F2 G replace baseline。**Residual gap** G1 marginal MISS 0.6pp + 2 NEW context_precision fails(Q-W25-T02 + Q-W25-I02)+ G4 control regression remains = R-W26-2 **partial residual exists**(D1.35 H2 attention dilution emerging primary residual axis;append mode 唔完全 close gap)。**W28+ candidates priority(per ADR-0038 §Decision #4)**:(b) `max_tokens_per_parent` sweep [HIGHEST — H2 direct intervention]+ (c) RAGAs orchestrator-aware judge tune [Second — H1+H2 judge side address per R-W26-2 original framing];若 (b) PASS close G1 + G4 gap → R-W26-2 fully decays。 |
