...
### F2 — Step 1 parent-document retrieval(PIVOTED 2026-05-25 D1 per F1 empirical refutation;original rerank threshold scope refactored — see §7 Plan Changelog)

> **⚠️ PIVOTED scope 2026-05-25 D1 per Chris pick (C)** — original「Step 1 rerank score threshold」scope was based on brief §3 方向 A premise「Cohere 排第 4/5 低分 chunk 照入 context → 唔相關內容」which F1 + threshold-probe empirically refuted(Cohere scores P25=0.83 / min=0.67 全部高分,passed vs failed queries 嘅 score distribution overlap highly)。Revised scope = brief §6 step 4 escalation:**parent-document / section-level retrieval ADR-0037**(brief §3 方向 E,🟡 H1 Tier 1 ceiling)。Full F2 deliverable rewrite deferred to next session per architectural design scope。Below 為 ORIGINAL spec(superseded):

#### ORIGINAL — Step 1 rerank score threshold(SUPERSEDED 2026-05-25 D1)
