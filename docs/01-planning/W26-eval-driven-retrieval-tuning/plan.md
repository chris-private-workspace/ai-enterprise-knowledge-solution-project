...
end_date: 2026-05-25   # same-day collapse (F0-F4 closed via 6 session segments 2026-05-25)
status: closed_partial  # per Chris pick (α) 2026-05-25 D5 — F2 → F3 gate FAIL on both G3 + G4; parent-doc default OFF preserved as W27+ candidate; F3 query expansion NOT triggered
spec_refs:
...
| **Step 2** Query expansion enable + eval | §3 方向 B implied(ADR-0034 framework already exists)| 🟢 conditional(gated on Step 1 + eval delta) — **NOT TRIGGERED 2026-05-25 D5** per F2 → F3 gate FAIL + Chris α pick | ADR-0034 框架已存,`enable_query_expansion: bool = False` default — Step 2 = flip flag for measurement experiment(NOT default flip),量 step 2 RAGAs delta;若 measurable improvement → 後續 sprint 考慮 default flip(自己 NEW Change/Phase);若 no improvement → close W26 with rationale 「query expansion not helpful at this point」。**實際 2026-05-25 D5**:F2 G RAGAs eval Step 1 parent-doc fail(G3 + G4 hard gates 雙 FAIL)→ Chris pick α → F3 conditional NOT triggered;query expansion 仍 default OFF + ADR-0034 framework preserved 留 W27+ standalone test 候選 |
...
| 2026-05-25 D5 | **F2 G RAGAs re-eval + H gate FAIL + Chris α pick + W26 closeout PARTIAL**(6th session segment same calendar day):**(a)** F2.19-F2.23 G RAGAs re-run 13 queries `eval-set-v0-w25-supplement` 492.1s + raw JSON + delta report `parent-doc-metrics-W26-D5.md` 完成;**(b)** Aggregated delta:recall_at_5 unchanged + **faithfulness -8.36pp** + **correctness -6.12pp** + p95_latency +18.7% within budget;**(c)** 2 critical regressions:Q-W25-I01 control「high-level architecture」full-pass → answer_relevancy=0.54 + context_recall=0(control 被破壞)+ Q-W25-I07 post-BUG-025 full-pass → **faithfulness=0.00 + answer_relevancy=0.00**(synthesizer 完全失敗);**(d)** G3 hard gate(context_recall improvement on 5-cohort)= **0/5 improvement + 1 new regression** → FAIL;G4 hard gate(faithfulness regression)= **-8.36pp** → FAIL severely;**(e)** F2 → F3 gate verdict FAIL → F3 conditional NOT triggered per plan §2 F3 gating;**(f)** Root cause hypothesis(4-axis):parent section LLM attention dilution + RAGAs faithfulness judge mismatch with citation invariant(LLM cites parent siblings outside top-5 reranked set)+ Q-W25-I07 likely REFUSAL_PHRASE trigger OR citation chunk_id drift + dispatch chain replace-vs-append architectural variable;**(g)** Chris AskUserQuestion α pick(Recommended)2026-05-25 D5:PARTIAL closeout + parent-doc default OFF preserved as W27+ candidate(per Q4 measurement-experiment-fail ≠ revert discipline);**(h)** `.env` `ENABLE_PARENT_DOC_RETRIEVAL=true` removed;backend Settings default restored;**(i)** F2 G commit `a35c1ed`;**(j)** F4 closeout cascade initiated — plan status `active → closed_partial` + §1.2 Step 2 row marked NOT TRIGGERED + retro Day 1 cont 6 entry + cross-doc sync(architecture.md §3.6 + COMPONENT_CATALOG + RISK_REGISTER + session-start.md §10/§11/§12) | Plan §2 F3 acceptance #4「no improvement / regression → PARTIAL + escalate W27+」rule triggers α decision path;ADR-0037 §Implementation Deliverables A-F 全 ship + tests 全綠 + governance 整全 — measurement-fail 唔觸 revert per Karpathy §1.3 surgical | Chris(α pick via AskUserQuestion)+ AI(F2 G eval + delta report + .env restore + F4 cascade)|
