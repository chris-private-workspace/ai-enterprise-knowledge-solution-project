...
- [x] **F2.12** — `frontend/app/(app)/traces/[traceId]/page.tsx` V6 Debug View 9→10 stages — **DONE 2026-05-25 D1 cont 5**(per Q5 Option A explicit insert「Parent-Document Retriever」stage card between Context Expander + LLM Synthesis,IA flat 之後實際路徑為 `/traces/[traceId]` 非 `/debug/[traceId]` per W18 ADR-0024;mockup 同步先行避 H7 violation):
  - [x] **F2.12.pre — H7 trigger surface + mockup sync first** — STOP and ask 用戶 → 揀方案 (2) sync mockup → 改 `references/design-mockups/`:`ekp-page-trace.jsx`(comment + line 50 「9-stage pipeline」→「10-stage pipeline」+ line 44 「all 9 stages passed」→「all 10 stages passed」+ line 304-310 categories Context `[6]`→`[6,7]` + Synthesis `[7,8]`→`[8,9]` + line 421 page-subtitle)+ `ekp-data.jsx` MOCK_TRACE comment + NEW stage 08 Parent-Document Retriever sample data(SPAN type,latency 95ms,cost 0,details requested_anchors=1 parents_fetched=1 siblings_aggregated=12 truncated_count=0 skipped_shallow_count=0 fetch_latency_ms=85)+ 既有 stage 8 「08 LLM Synthesis」→「09 LLM Synthesis」+ stage 9 「09 Final Response」→「10 Final Response」+ DESIGN_README.md row + EKP Platform.html tweak label + ekp-page-auth.jsx 2 hero strings + ekp-page-labs-1.jsx labs subtitle + ekp-page-settings-tabs.jsx Langfuse provider role + ekp-shell.jsx sidebar hint + PAGE_INVENTORY.md row 10 + section 102 — **共 7 mockup files synced**;2 處「pre-W26 F2 baseline was 9 stages」歷史 context comment 保留作 traceability
  - [x] **F2.12a** — Observation-name prefix add in `PIPELINE_STAGES` array — NEW entry id=8 `obsPrefixes: ['generation.parent_doc_retrieval']`(對齊 backend `parent_doc_retriever.py:_STAGE_NAME` constant)
  - [x] **F2.12b** — Per-stage data display 由既有 stage rendering 邏輯統一處理(existing `StageRowData.details` from buckets → key/value list 既有 mechanism cover `requested_anchors` / `parents_fetched` / `siblings_aggregated` / `truncated_count` / `skipped_shallow_count` 同 ADR-0020 context_expansion 同 schema)
  - [x] **F2.12c** — H7 self-verify per CLAUDE.md §3.2.1 + §5.7 ALL passed:
    - **Layout 結構**:NEW stage 8 entry mirror 既有 stage entries(id + name + description + obsPrefixes + note)— object shape identical
    - **Spacing / typography**:既有 stage card iteration 機制由 `PIPELINE_STAGES.map` 統一渲染,NEW stage 共享 token / spacing — 0 visual divergence
    - **Color tokens**:Context category 由 `[7]` → `[7, 8]` 擴展(Option B per ADR-0037 §6.1)— Parent-Document 同 Context Expander 共享 green color oklch(0.65 0.14 145)反映兩者同屬 post-rerank context aggregation 架構意圖
    - **Interaction states**:hover / expand / data-active 等狀態 由既有 `expanded` state + `setExpanded` 統一管理 — NEW stage 0 額外 wiring 需要
    - **Responsive breakpoints**:既有 mockup mobile / desktop pattern 跟住 — `PIPELINE_STAGES.length` 由 9 → 10 唔 break 既有 stat-grid + viz 三模式(vertical / waterfall / flame)
    - **A11y affordances**:既有 `role="tab"` + `aria-selected` + Vertical / Waterfall / Flame 三模式 seg button accessible — NEW stage 0 影響
    - **Mockup 字面對齊**:mockup `ekp-page-trace.jsx` + `ekp-data.jsx` 已 sync 到 10-stage 字面 + 8th stage data shape;page.tsx 100% mirror updated mockup(無 approximate)
  - [x] **F2.12d** — Title sync:line 516 `"9-stage pipeline"` → `"10-stage pipeline"`(對齊 mockup line 50)
  - [x] **F2.12e** — Categories sync:Context `stageIds: [7]` → `[7, 8]` + Synthesis `[8, 9]` → `[9, 10]`(對齊 mockup line 308-309)
  - [x] **F2.12f** — Docstring update — 標明 W26 F2.12 2026-05-25 9→10 expansion + ADR-0037 §Q5 Option A explicit insert + Context category extension rationale + mockup sync 100% mirror reference
