...
### Finding D — 圖片按文件次序排(問題1 presentation 修法)
- **診斷**:GL03 程序 = §3.1.3 Create(~p20-28,24 圖)→ §3.1.4 Approve → §3.1.5 Post(~p31,16 圖)。兩段圖都返咗,但 reranker 將 §3.1.5「Post」排第一(query「post」字面 match),`dedupeCitationImages` 按 relevance 排 → gallery lead 住 p31。**唔係 recall 缺失,係排序**。
- **用戶選擇**:「按文件次序排圖」。
- **修復**:`dedupeCitationImages` 加 stable sort by `imageSectionPath`(image 自己 section)→ document order(Create→Approve→Post);badge 維持 citation 位置。純前端。
- **Live 確認(數據模擬真實 /query)**:修前頭 8 張全 §3.1.5(p31);修後頭 8 張 = §3.1.1 Overview + §3.1.3 Create(p20-28)lead。✅
- **測試**:`citation-images.test.ts` 加 2 個(document order + stable same-section);21/21 pass。
- **仍 out-of-scope**:retrieval RELEVANCE 本身(reranker 偏好 §3.1.5)不改 —— text 已完整覆蓋全程序,圖片排序已解決 UX;深層 rerank 調優屬獨立課題。

### Commits(addendum)
| Hash | Subject |
|---|---|
| `154dfce` | fix(chat): BUG-034 Finding C — em-dash for expansion-citation sentinel score |
| _(Finding D)_ | fix(chat): BUG-034 Finding D — order citation images by document position |

---

**End of BUG-034 progress (Finding C+D verifying — 待用戶 chat 肉眼確認 + merge)**
