...
**Wave C shipped(post-W22)**:`/users` Tier 1.5 RBAC ✅ **implemented W24c**(Wave C3 per ADR-0027 Option A)+ `/settings` 6-tab fully-editable ✅ **implemented W24+W24b**(Wave C1+C2 per ADR-0026)。**Tier 2 boundary preserved**:custom role creation + Power User activation + multi-tenancy + all Tier 2 affordances catalogued in W19 F5 27-affordance catalog rendered as `<DisabledAffordance>` per H4 boundary discipline。
...
| 5 | `/kb/[id]` | `ekp-page-kb.jsx` `PageKbDetail` | C09 §5.5 (8 tabs) | ✅ **Implemented W20 F5 (7-tab) → W24c F10 Access tab activated (8-tab full)**: Documents · Chunks · Images · Chunking Lab · Pipeline · Retrieval Testing · Settings · **Access**. **W24c F10**(per ADR-0025 + ADR-0027 Option A RBAC infra): Access tab disabled affordance → **active 8th tab** — NEW `<TabKbAccess>`(banner + 4-stat + Visibility card read-only + per-KB ACL Members table)consume `kbApi.listAcl`(`GET /kb/{kb_id}/acl`). **W43 F3.1 design-stage expansion**(per ADR-0040): Settings tab +2 cards — **Advanced retrieval tuning**(12 per-KB runtime knobs, 3 groups 檢索/引用/圖片, 繼承/覆寫 state)+ **config-test 試跑 panel**(query + 1-5 runs + A/B vs saved + variance band). Backend = `KbConfig` 12 Optional knobs + `POST /kb/{id}/config-test`. | `KbStatus` (W20 F5.1 +archived flag), `ChunkRecord`, `ImageRef` (W20 F5.2 aggregation), `KbMetadataPatch`, `KbAclEntry` (W24c F10), `KbConfig` (W43 +12 knobs), `ConfigTestRequest`/`ConfigTestResult` (W43) |
...
| 10 | `/traces/[traceId]` | `ekp-page-trace.jsx` `PageTrace` | C09 §5.7 + ADR-0020 V6 + **ADR-0037 W26 F2 10-stage(stage 08 Parent-Document Retriever inserted between Context Expander + LLM Synthesis)** | ✅ **Implemented W22 F7.3 + W26 F2.12 mockup sync 2026-05-25**(strict-fidelity rebuild per mockup 3 viz modes vertical / waterfall / flame + Open Langfuse link;ADR-0037 §Q5 Option A explicit 10-stage insert);full data-seeded viz E2E deferred W24+ per BUG-004 (needs seeded Langfuse trace via Track A IT cred + CO17). | `TraceDetail`, `TraceStage` |
...
| 12 | `/users` | `ekp-page-users.jsx` | C16 Users Service (Tier 1.5, NEW) | ✅ **Implemented W24c-users-rbac (Wave C3) per ADR-0027 Option A full RBAC**. **4 tabs**: Members · Roles & permissions · Groups · Audit log — NET NEW route + `useRole()` hook + whole-page Workspace-Admin role-gating. Tier 1 has 3 roles (Admin / Editor / User); Power User = Tier 2 disabled affordance per H4. Backed by 5 NEW Postgres tables + 13 NEW endpoints across 4 routers + ACL middleware. **Activated `/kb/[id]` Access tab** (F10). mockup-presentational CRUD affordance reproduced inert (Karpathy §1.2); `kb_acl` CRUD write UI 🚧 W24d+. | `UserSummary`, `Role`, `RolePermission`, `Group`, `KbAclEntry`, `AuditLogEntry` (5 NEW Postgres tables: `roles` + `role_permissions` + `groups` + `group_members` + `kb_acl`) |
...
7. `/traces/[traceId]` 10-stage Debug View (3 viz modes — start with vertical;stage 08 Parent-Document Retriever per ADR-0037 W26 F2)
