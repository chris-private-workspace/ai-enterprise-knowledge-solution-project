...
- **Azure Container Apps logs**(primary CLI path):`az containerapp logs show --name ekp-beta-backend --resource-group ekp-beta-rg --tail 100 --follow`(live tail);`--type system` for system events
- **🔴 R8 corp proxy fallback for `az containerapp logs show`(W11 D2 calibration 2026-06-10)**:`az containerapp logs show` 透過 `eastus2.azurecontainerapps.dev` 數據平面 hostname — Ricoh corp proxy 可能 block(R8 truststore mitigation 對 management plane `management.azure.com` work,但 data plane 唔同 cert chain;W11 D2 calibration verified `az containerapp exec` viable through management plane,**but** `az containerapp logs show` blocked through data plane)。**Alternative path** — Log Analytics workspace REST API via httpx truststore-mitigated client:query `ContainerAppConsoleLogs_CL` table via `POST https://api.loganalytics.io/v1{LA-WORKSPACE-RESOURCE-ID}/query` w/ AAD Bearer token scoped to `api.loganalytics.io`(KQL example:`ContainerAppConsoleLogs_CL | where ContainerAppName_s == "ekp-beta-backend" | top 100 by TimeGenerated desc | project TimeGenerated, Log_s, RevisionName_s`)。**Tradeoff**:LA ingestion latency ~2-5 min(vs CLI live tail real-time)— acceptable for incident triage,not real-time monitoring。**Calibration**:W11 D2 R8 calibration data accumulated(8 incidents bypassed via httpx-truststore path,session 2026-06-10);once corp IT escalates `*.azurecontainerapps.dev` allowlist + truststore inclusion,CLI direct path resumes(R8 closure trigger per `RISK_REGISTER.md` R8)。**Operational note**:呢條 fallback 用於 retroactive log query;real-time monitoring path during incident = Langfuse generations API(application-side,unaffected by data plane R8)+ `/observability/cost-summary` dashboard polling
- **Common causes**:
...
| 2026-06-10(W11 D2 cont)| **§2 Root cause investigation R8 fallback note added** — closes W11 D2 F4.5 LIVE exercise carry-over (3) | Added 2 new bullets to §2 Root cause investigation:(a)primary `az containerapp logs show` CLI path documented;(b)R8 corp proxy fallback explicit — Log Analytics workspace REST API via httpx truststore-mitigated client query `ContainerAppConsoleLogs_CL` table(`api.loganalytics.io/v1` endpoint w/ AAD Bearer token,KQL example included)+ tradeoff(LA ingestion latency ~2-5 min vs CLI real-time)+ calibration data(8 W11 D2 incidents bypassed via this path)+ closure trigger(corp IT `*.azurecontainerapps.dev` allowlist + truststore inclusion → CLI direct path resumes)。**Operational note added**:fallback used for retroactive log query only;real-time monitoring path during incident = Langfuse generations API(application-side,unaffected by data plane R8)。Carry-over (3) from W11 D2 F4.5 LIVE exercise(2026-06-10 `20f7f56` outcome row above)now closed — minor doc enhancement landed within W11 D2 cont window per Karpathy §1.4 goal-driven completion(non-blocking governance) |
